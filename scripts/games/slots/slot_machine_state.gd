...
static func normalize(machine_value: Variant) -> Dictionary:
	var machine: Dictionary = JsonCoerceScript._copy_dict(machine_value)
...
static func normalize_owned(machine: Dictionary) -> Dictionary:
	...
	machine["reel_heights"] = JsonCoerceScript._int_array(machine.get("reel_heights", []))
	...
	machine["reel_stops"] = JsonCoerceScript._int_array(machine.get("reel_stops", []))
	...
	machine["last_reels"] = JsonCoerceScript._copy_array(machine.get("last_reels", []))
	...
	var last_bonus_replay: Dictionary = JsonCoerceScript._copy_dict(machine.get("last_bonus_replay", {}))
	machine["last_bonus_replay"] = _normalize_active_bonus(last_bonus_replay) if not last_bonus_replay.is_empty() else {}
	machine["last_tease_events"] = JsonCoerceScript._copy_array(machine.get("last_tease_events", []))
	machine["last_nudge_offer"] = JsonCoerceScript._copy_dict(machine.get("last_nudge_offer", {}))
	...
	machine["pinball_feature_state"] = JsonCoerceScript._copy_dict(machine.get("pinball_feature_state", {}))
	machine["buffalo_feature_state"] = JsonCoerceScript._copy_dict(machine.get("buffalo_feature_state", {}))
	machine["slot_item_state"] = JsonCoerceScript._copy_dict(machine.get("slot_item_state", {}))
	return machine
...
static func canonical_geometry(definition: Dictionary, family_id: String, format_id: String) -> Dictionary:
	for format_value in JsonCoerceScript._dictionary_array(definition.get("slot_formats", [])):
		...
		if str(format.get("id", "")) != format_id:
			continue
		var geometries: Dictionary = JsonCoerceScript._copy_dict(format.get("geometry", {}))
		var geometry: Dictionary = JsonCoerceScript._copy_dict(geometries.get(family_id, {}))
...
static func behavior_combo_count(definition: Dictionary) -> int:
	return JsonCoerceScript._dictionary_array(definition.get("slot_formats", [])).size() * JsonCoerceScript._dictionary_array(definition.get("slot_types", [])).size() * JsonCoerceScript._dictionary_array(definition.get("slot_math_variants", [])).size() * JsonCoerceScript._dictionary_array(definition.get("slot_bonus_variants", [])).size()


static func visual_machine_count(definition: Dictionary) -> int:
	return behavior_combo_count(definition) * JsonCoerceScript._dictionary_array(definition.get("slot_cabinet_variants", [])).size()


static func _normalize_bet_ladder(value: Variant) -> Dictionary:
	var ladder: Dictionary = JsonCoerceScript._copy_dict(value)
	var selected_id := str(ladder.get("selected_id", "bet_2"))
...
static func _normalize_bonus_state(value: Variant) -> Dictionary:
	var source: Dictionary = JsonCoerceScript._copy_dict(value)
	var per_bet: Dictionary = JsonCoerceScript._copy_dict(source.get("per_bet", {}))
	for option_value in BET_OPTIONS:
		var option: Dictionary = option_value
		var bet_id := str(option.get("id", ""))
...
static func _normalize_per_bet_bucket(value: Variant) -> Dictionary:
	var bucket: Dictionary = JsonCoerceScript._copy_dict(value)
	return {
...
static func _normalize_active_bonus(value: Variant) -> Dictionary:
	...
	active["history"] = JsonCoerceScript._copy_array(active.get("history", []))
	active["choices"] = JsonCoerceScript._copy_array(active.get("choices", []))
	active["shot_queue"] = JsonCoerceScript._copy_array(active.get("shot_queue", []))
	active["locks"] = JsonCoerceScript._copy_array(active.get("locks", []))
	...
	active["launch_start"] = JsonCoerceScript._copy_dict(active.get("launch_start", {}))
	...
	active["collected_coins"] = JsonCoerceScript._copy_array(active.get("collected_coins", []))
	active["last_collected_coins"] = JsonCoerceScript._copy_array(active.get("last_collected_coins", []))
	...
	active["coin_reveals"] = JsonCoerceScript._copy_array(active.get("coin_reveals", []))
	...
	active["retrigger_threshold"] = maxi(1, int(active.get("retrigger_threshold", 3)))
	active["retrigger_grant"] = maxi(1, int(active.get("retrigger_grant", 8)))
	active["max_total_steps"] = maxi(int(active.get("total_steps", 0)), int(active.get("max_total_steps", 60)))
	...
	active["physics"] = JsonCoerceScript._copy_dict(active.get("physics", {}))
	active["runtime_session_id"] = str(active.get("runtime_session_id", ""))
	active["runtime_seed"] = int(active.get("runtime_seed", 0))
	active["board_id"] = str(active.get("board_id", ""))
	active["pinball_summary"] = JsonCoerceScript._copy_dict(active.get("pinball_summary", {}))
	active["pinball_view"] = JsonCoerceScript._copy_dict(active.get("pinball_view", {}))
	active["pinball_item_effects"] = JsonCoerceScript._copy_dict(active.get("pinball_item_effects", {}))
	active["pinball_item_hooks"] = JsonCoerceScript._copy_array(active.get("pinball_item_hooks", []))
	active["sequencer_state"] = JsonCoerceScript._copy_dict(active.get("sequencer_state", {}))
	active["sequence_events"] = JsonCoerceScript._copy_array(active.get("sequence_events", []))
	active["lit"] = JsonCoerceScript._copy_dict(active.get("lit", {}))
	active["combo_state"] = JsonCoerceScript._copy_dict(active.get("combo_state", {}))
	active["pinball_debug"] = JsonCoerceScript._copy_dict(active.get("pinball_debug", {}))
	active["guided_assist_count"] = maxi(0, int(active.get("guided_assist_count", 0)))
	active["event_log"] = JsonCoerceScript._copy_array(active.get("event_log", []))
	active["trajectory"] = JsonCoerceScript._copy_array(active.get("trajectory", []))
	active["display_event_log"] = JsonCoerceScript._copy_array(active.get("display_event_log", []))
	active["display_trajectory"] = JsonCoerceScript._copy_array(active.get("display_trajectory", []))
	...
	active["live_input"] = JsonCoerceScript._copy_dict(active.get("live_input", {}))
	...
	active["video_targets"] = JsonCoerceScript._copy_dict(active.get("video_targets", {}))
	...
	active["collection_meter"] = JsonCoerceScript._copy_dict(active.get("collection_meter", {}))
	active["fill_meter"] = JsonCoerceScript._copy_dict(active.get("fill_meter", {}))
	active["jackpot_ladder"] = JsonCoerceScript._copy_dict(active.get("jackpot_ladder", {}))
	active["last_lock_events"] = JsonCoerceScript._copy_array(active.get("last_lock_events", []))
	...
	active["trophy_choices"] = JsonCoerceScript._copy_array(active.get("trophy_choices", []))
	active["trophy_reveals"] = JsonCoerceScript._copy_array(active.get("trophy_reveals", []))
