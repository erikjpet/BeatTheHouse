...
static func draw_card(surface, card_value: Variant, rect: Rect2, options: Dictionary = {}) -> void:
	draw_card_state(
		...
		card_value,
		rect,
		bool(options.get("held", false)),
		bool(options.get("suggested", false)),
		bool(options.get("scoring", false))
	)


static func draw_card_state(surface, card_value: Variant, rect: Rect2, held: bool = false, suggested: bool = false, scoring: bool = false) -> void:
