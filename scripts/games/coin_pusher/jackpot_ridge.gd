...
static func initial_state(config: Dictionary, rng: RngStream, lane_count: int, cell_count: int) -> Dictionary:
	...
	var state := {
		...
		"multiplier_lock_until_phase_units": -1,
...
static func finish_drop(state: Dictionary, stroke_cycle: int, phase_fp: int, period_ticks: int = 240) -> void:
	var remaining: Array = []
	var absolute_phase := stroke_cycle * maxi(1, period_ticks) * 1000 + phase_fp
	var protected_multiplier := payout_multiplier(state) if absolute_phase < int(state.get("multiplier_lock_until_phase_units", -1)) else 0
	var protection_used := false
	for value in state.get("armed_multipliers", []):
		if typeof(value) != TYPE_DICTIONARY:
			continue
		var charge := (value as Dictionary).duplicate(false)
		if not protection_used and protected_multiplier > 1 and int(charge.get("multiplier", 1)) == protected_multiplier:
...
static func apply_physical_events(state: Dictionary, physics_events: Array, config: Dictionary, fallback_cycle: int = 0) -> Dictionary:
	...
	for event_value in physics_events:
		...
		outcome_phases[str(metadata.get("feature_id", ""))] = int(event.get("phase_fp", 0))
	for value in pucks:
		if typeof(value) != TYPE_DICTIONARY:
			continue
	...
	for puck in banked:
		match str((puck as Dictionary).get("kind", "")):
			...
			"lock":
				if payout_multiplier(state) > 1:
					var lock_cycle := int(outcome_cycles.get(str((puck as Dictionary).get("id", "")), fallback_cycle))
					var lock_phase := int(outcome_phases.get(str((puck as Dictionary).get("id", "")), 0))
					var period := maxi(1, int(config.get("stroke_period_ticks", 240)))
					var until_phase_units := lock_cycle * period * 1000 + lock_phase + period * 1000
					state["multiplier_lock_until_phase_units"] = maxi(int(state.get("multiplier_lock_until_phase_units", -1)), until_phase_units)
			"dud":
				pass
	...
	if ridge_triggered:
		...
		# The cycle containing the third bank is the trigger cycle, not one of the
		# three promised full-speed cycles. Anchor consumption at the following
