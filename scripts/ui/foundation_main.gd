...
func _blackjack_host_ledger(candidate: RunState, create: bool = true) -> Dictionary:
	if candidate == null or current_game == null:
		return {}
	...
	var validated := BlackjackActionAuthorityScript.validate_persisted_ledger(table.get(BlackjackActionAuthorityScript.LEDGER_KEY, {}), binding)
	if not validated.is_empty() or not create:
		return validated
	return BlackjackActionAuthorityScript.default_ledger(binding)
...
func _blackjack_host_surface_intent(surface_action: String, index: int, confirm_requested: bool = false, surface_time_msec: int = -1) -> Dictionary:
	...
	if bool(command.get("handled", false)):
		...
		if bool(command.get("direct_resolve", false)) or bool(command.get("resolve", false)):
			...
			command["_blackjack_host_delivery_key"] = str(delivery.get("request_key", ""))
...
func _blackjack_host_replay_request(request_key: String) -> Dictionary:
	...
	if not cache.has(request_key) or typeof(cache.get(request_key)) != TYPE_DICTIONARY:
		return _blackjack_host_rejection("unknown_receipt", "Blackjack request receipt is unavailable.", request_key)
	return ((cache[request_key] as Dictionary).get("response", {}) as Dictionary).duplicate(true)


func _blackjack_host_prepare_delivery(action_id: String, stake: int, requested_key: String = "") -> Dictionary:
	var candidate := _blackjack_host_detached()
	if candidate == null:
	...
	if not requested_key.is_empty() and cache.has(requested_key):
		var cached_entry: Dictionary = cache.get(requested_key, {})
		if str(cached_entry.get("action_id", "")) != action_id:
		...
		return {"ok": true, "cached_response": (cached_entry.get("response", {}) as Dictionary).duplicate(true)}
	...
	if not pending.is_empty():
		if not requested_key.is_empty() and str(pending.get("request_key", "")) != requested_key:
...
func _blackjack_host_proposal_valid(proposal: Dictionary, proposal_input: Dictionary) -> bool:
	...
	return provided_output_fingerprint == GameRitualRuntimeScript.canonical_fingerprint(output)


func _blackjack_host_resolve_intent(action_id: String, stake: int, delivery_key: String = "") -> Dictionary:
	if not _current_game_uses_blackjack_action_authority() or run_state == null or action_id.is_empty():
		return _blackjack_host_rejection("invalid_intent", "Blackjack action intent is unavailable.", delivery_key)
	var prepared := _blackjack_host_prepare_delivery(action_id, stake, delivery_key)
	...
	var proposed_ledger := _blackjack_host_ledger(proposed_candidate, false)
	...
	var receipt := BlackjackActionAuthorityScript.receipt_for(delivery, binding, result)
	...
	if not bool(result.get("defer_bankroll_zero_failure", false)):
		proposed_candidate.advance_environment_turns(1)
	...
	proposed_ledger = _blackjack_host_ledger(proposed_candidate, false)
	...
	result["blackjack_host_apply_receipt"] = BlackjackActionAuthorityScript.receipt_for(delivery, binding, result)
	result["blackjack_host_content_fingerprint"] = BlackjackActionAuthorityScript.result_fingerprint(result)
	proposed_ledger = BlackjackActionAuthorityScript.commit_response(proposed_ledger, delivery, result)
...
func _apply_game_surface_command(command: Dictionary, index: int = -1, confirm_requested: bool = false, notify_coach: bool = true, input_route_guarded: bool = false) -> bool:
	...
	var blackjack_delivery_key := str(command.get("_blackjack_host_delivery_key", ""))
	if direct_resolve and not action_id.is_empty():
		_resolve_game_action(action_id, bool(command.get("skip_stake_validation", false)), bool(command.get("preserve_surface_ui_state", false)), false, resolved_surface_ui_state, true, direct_stake_override, blackjack_delivery_key)
		return true
	if not action_id.is_empty() and not action_kind.is_empty():
		...
		if bool(command.get("resolve", false)) or confirm_requested or already_selected:
			_resolve_game_action(action_id, bool(command.get("skip_stake_validation", false)), bool(command.get("preserve_surface_ui_state", false)), false, resolved_surface_ui_state, true, 0, blackjack_delivery_key)
			return true
...
func _resolve_game_action(action_id: String, skip_stake_validation: bool = false, preserve_surface_ui_state: bool = false, wager_confirmed: bool = false, resolved_surface_ui_state: Dictionary = {}, input_route_guarded: bool = false, stake_override: int = 0, blackjack_delivery_key: String = "") -> void:
	...
	if _current_game_uses_blackjack_action_authority():
		result = _blackjack_host_resolve_intent(action_id, stake, blackjack_delivery_key)
	else:
...
func _record_tutorial_action_if_authored(action_id: String) -> void:
	...
	for lesson_value in library.tutorial_lessons:
		...
		match str(completion.get("type", "")):
			...
			"one_of_actions":
				authored = _string_array(completion.get("action_ids", [])).has(clean_action_id)
		if authored:
			break
	if not authored:
		return
...
func _complete_preperformed_tutorial_actions() -> void:
	...
	for _index in range(16):
		...
		match str(completion.get("type", "")):
			...
			"one_of_actions":
				action_ids = _string_array(completion.get("action_ids", []))
			_:
				return
		var matched_action := ""
