...
static func draw(canvas: CanvasItem, rect: Rect2, object_data: Dictionary, accent: Color, selected: bool, disabled: bool, flicker: float) -> void:
	...
	Kit.draw_die(canvas, Rect2(start, die_size), _die_value(visual, 0, 6))
	Kit.draw_die(canvas, Rect2(start + Vector2(die_size.x * 1.12, -die_size.y * 0.18), die_size), _die_value(visual, 1, 5))
	Kit.draw_die(canvas, Rect2(start + Vector2(die_size.x * 2.24, die_size.y * 0.08), die_size), _die_value(visual, 2, 4))
	Kit.draw_die(canvas, Rect2(start + Vector2(die_size.x * 3.36, -die_size.y * 0.12), die_size), _die_value(visual, 3, 3))
	Kit.draw_die(canvas, Rect2(start + Vector2(die_size.x * 4.48, die_size.y * 0.12), die_size), _die_value(visual, 4, 2))
...
static func _draw_small(canvas: CanvasItem, safe: Rect2) -> void:
	...
	for index in range(5):
		...
		Kit.draw_die(canvas, die, 6 - index)
