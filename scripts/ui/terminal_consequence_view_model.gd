...
static func failure_summary(run_state: RunState, data: Dictionary) -> Dictionary:
	if run_state == null:
		return {}
	...
	var story_lines: Array = data.get("story_lines", [])
	var item_lines: Array = ["Inventory: empty."] if inventory_items.is_empty() else ["Inventory: %s." % inventory_summary(inventory_items)]
	if not inventory_items.is_empty():
		item_lines.append_array(inventory_items.slice(0, 5))
	var debt_lines: Array = ["Debt: none."] if debt_items.is_empty() else ["Debt: %s." % debt_summary(debt_items)]
	...
	if travel_choices.is_empty():
		...
		travel_lines.append("Routes at failure: %s." % travel_summary(travel_choices))
		for choice_value in travel_choices.slice(0, 5):
			if typeof(choice_value) != TYPE_DICTIONARY:
				continue
			var choice: Dictionary = choice_value
			var line := str(choice.get("label", choice.get("id", "Route")))
			if choice.has("cost"):
				line += " cost %d" % int(choice.get("cost", 0))
			if not bool(choice.get("enabled", true)):
				line += " locked: %s" % str(choice.get("disabled_reason", "unavailable"))
			travel_lines.append(line)
	var recent_result_lines: Array = ["No recent action was recorded."] if recent_message.strip_edges().is_empty() else [recent_message]
	...
	var alcohol_lines := [
		"Alcohol: %s, drunk %d, need %d." % [run_state.alcohol_condition_label().capitalize(), run_state.drunk_level, run_state.alcoholic_level],
	...
	var visited_places: Array = data.get("visited_places", [])
	...
	return {
		"title": title,
		"message": str(data.get("player_facing_message", message)),
		"reason": reason,
		"reason_label": _call_label(label_from_id, reason) if not reason.is_empty() else "Run failed",
		...
		"score_lines": terminal_score_lines(score_summary),
		...
		"environment_kind": _call_label(label_from_id, str(environment.get("kind", environment.get("archetype_id", "room")))),
		...
		"bag_lines": data.get("bag_lines", []),
		"debt_lines": debt_lines,
		"recent_result_lines": recent_result_lines,
		"story_lines": story_lines,
		"flag_lines": data.get("flag_lines", []),
	}


static func victory_summary(run_state: RunState, data: Dictionary) -> Dictionary:
	if run_state == null:
		return {}
	var pressure: Dictionary = data.get("pressure", {})
	...
	if not inventory_items.is_empty():
		if not run_state.active_item_id.is_empty():
			item_lines.append("Active item: %s." % _call_label(label_from_id, run_state.active_item_id))
		item_lines.append_array(inventory_items.slice(0, 5))
	...
	return {
		...
		"message": message,
		"route": route,
		"route_label": _call_label(label_from_id, route),
		"run_status": run_state.run_status,
		"seed": run_state.player_facing_seed_text(),
		"seed_hidden": run_state.seed_is_hidden(),
		"score_spending": int(score_summary.get("base_spending", 0)),
		...
		"visited_count": visited_places.size(),
		"visited_summary": ", ".join(visited_places) if not visited_places.is_empty() else "current room only",
		"next_act_line": victory_next_act_line(route),
		"alcohol_lines": alcohol_lines,
		"item_lines": item_lines,
		"bag_lines": data.get("bag_lines", []),
		"debt_lines": debt_lines,
...
static func terminal_score_lines(score: Dictionary) -> Array:
	var base := int(score.get("base_spending", 0))
	var multiplier := maxi(1, int(score.get("multiplier", 1)))
	return ["Items and travel spent: %d" % base, "Victory multiplier: x%d" % multiplier, "Run score: %d" % int(score.get("score", base * multiplier))]


static func victory_next_act_line(route: String) -> String:
	match route:
		RunState.GRAND_CASINO_HIGH_ROLLER_EVENT_ID: return "The Players Card opens quieter rooms. Your name is now on the list."
		RunState.GRAND_CASINO_SHOWDOWN_ROUTE: return "Rourke lets the elevator close. The house will remember your face."
	return "The house closes one door and leaves another cracked open."


static func visited_environment_summary_lines(run_state: RunState) -> Array:
	var labels: Array = []
	if run_state == null: return labels
	for value in run_state.environment_history:
		if typeof(value) != TYPE_DICTIONARY: continue
		var label := str((value as Dictionary).get("display_name", (value as Dictionary).get("id", "")))
		if not label.is_empty() and not labels.has(label): labels.append(label)
	var current := str(run_state.current_environment.get("display_name", run_state.current_environment.get("id", "")))
	if not current.is_empty() and not labels.has(current): labels.append(current)
	return labels
