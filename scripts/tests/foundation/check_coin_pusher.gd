...
func _check_pusher_v3_10_hold_inputs(library: ContentLibrary, game_definition: Dictionary, failures: Array) -> void:
	...
	canvas.call("_gui_input", key_press)
	canvas.hide()
	# Headless Controls do not receive the Window-driven visibility transition
	# consistently, so exercise the same state seam used by the notification.
	canvas.call("_sync_surface_visibility", false)
	canvas.show()
	canvas.call("_gui_input", key_release)
	canvas.call("_gui_input", key_press)
	canvas.call("set_environment_activity_paused", true)
	canvas.call("_gui_input", key_release)
	canvas.call("set_environment_activity_paused", false)
	if phases != ["begin", "end", "begin", "end", "begin", "end", "begin", "end", "begin", "cancel", "begin", "cancel", "begin", "cancel"]:
		failures.append("pusherv3_10 shared hold input did not preserve mouse/touch/keyboard/controller equivalence and focus/visibility/pause cancellation: %s." % JSON.stringify(phases))
	canvas.free()
