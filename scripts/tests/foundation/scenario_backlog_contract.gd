...
static func _check_event(library: ContentLibrary, archetype: Dictionary, scenario: Dictionary, layer_id: String, event_id: String, failures: Array) -> void:
	...
	for choice_value in choices:
		...
		var environment := EnvironmentInstanceScript.from_archetype_layer(archetype, layer_id, 2, run_state.create_rng("environment"), library, {}, scenario).to_dict() if not layer_id.is_empty() else EnvironmentInstanceScript.from_archetype(archetype, 1, run_state.create_rng("environment"), library, {}, scenario).to_dict()
