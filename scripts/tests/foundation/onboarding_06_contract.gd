...
static func _check_contextual_lesson_matrix(library: Variant, failures: Array) -> void:
	...
	var room_target := Rect2(448, 288, 384, 144)
	depth_context["anchor_rects"] = {"interactable_objects": {"fixture:center": room_target}}
	var depth_model := CoachViewModelScript.build(depth_lesson, depth_context)
	var depth_bubble := CoachViewModelScript._rect(depth_model.get("bubble_rect", {}))
	if depth_bubble.intersects(room_target):
		failures.append("Normal-run advice did not move away from public interaction geometry when a clear placement existed.")
...
static func _check_normal_tip_action_handoff(library: Variant, failures: Array) -> void:
	...
	if overlay.ok_button == null or overlay.ok_button.mouse_filter != Control.MOUSE_FILTER_STOP:
		failures.append("Normal-run advice lost its clickable explicit dismiss control.")
	if not _normal_tip_pointer_blockers(overlay, overlay.ok_button).is_empty():
	...
	if overlay.ok_button == null or overlay.ok_button.mouse_filter != Control.MOUSE_FILTER_STOP:
		failures.append("Replacement normal-run advice lost its clickable explicit dismiss control.")
	if not _normal_tip_pointer_blockers(overlay, overlay.ok_button).is_empty():
...
static func _normal_tip_pointer_blockers(node: Node, dismiss_button: BaseButton) -> Array:
	var blockers: Array = []
	if node == null:
		return ["missing_panel"]
	if node is Control and node != dismiss_button and (node as Control).mouse_filter != Control.MOUSE_FILTER_IGNORE:
		blockers.append((node as Control).name)
	for child in node.get_children():
		blockers.append_array(_normal_tip_pointer_blockers(child, dismiss_button))
	return blockers
