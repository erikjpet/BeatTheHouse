...
func _check_slot_spin_and_host_ack(library) -> void:
	...
	var turn_before_spin: int = int(run_state.current_environment.get("turns", 0))
	...
	_check(int(run_state.current_environment.get("turns", 0)) == turn_before_spin + 1, "Slot normal action skipped the host-owned environment turn.")
	var skip_host = FoundationMainScript.new()
	skip_host.set("current_game", slot)
	var hostile_skip_result := {ActionAuthorityScript.SKIP_ENVIRONMENT_TURN_KEY: true}
	_check(not bool(skip_host.call("_sealed_action_host_normalize_environment_turn", hostile_skip_result, "spin")) and not hostile_skip_result.has(ActionAuthorityScript.SKIP_ENVIRONMENT_TURN_KEY), "Slot hostile normal-action result gained environment-turn authority.")
	var authentic_skip_result := {ActionAuthorityScript.SKIP_ENVIRONMENT_TURN_KEY: false}
	_check(bool(skip_host.call("_sealed_action_host_normalize_environment_turn", authentic_skip_result, "slot_handpay_acknowledge")) and not authentic_skip_result.has(ActionAuthorityScript.SKIP_ENVIRONMENT_TURN_KEY), "Slot host-owned acknowledgement lost its exact turn-skip policy.")
	skip_host.free()
	...
	var ack_rng_before: int = int(run_state.rng_state)
	var ack_turn_before: int = int(run_state.current_environment.get("turns", 0))
	var accepted := _host_resolve(slot, run_state, "slot_handpay_acknowledge", 0)
	_check(bool(accepted.get("ok", false)) and bool(accepted.get("slot_handpay_acknowledgement", false)), "Slot did not seal the attendant acknowledgement: %s" % RuntimeScript.canonical_json(accepted))
	_check(run_state.bankroll == ack_cash_before and run_state.rng_state == ack_rng_before and int(run_state.current_environment.get("turns", 0)) == ack_turn_before and not bool(accepted.get("sealed_action_authoritative", false)), "Slot acknowledgement changed settlement, RNG, or environment-turn authority.")
...
func _check_video_poker_targeted_inputs(library) -> void:
	...
	var pending_machine: Dictionary = poker.call("_machine_state", run_state, run_state.current_environment)
	var pending_result: Dictionary = (pending_machine.get("last_result", {}) as Dictionary).duplicate(true)
	pending_result["double_credits"] = maxi(1, int(pending_result.get("win_credits", 1)))
	pending_machine["last_result"] = pending_result
	poker.call("_update_environment_state", run_state.current_environment, pending_machine)
	var hidden_ui := {"double_active": true}
	var hidden_surface_a: Dictionary = poker.surface_state(run_state, run_state.current_environment, hidden_ui)
	var hidden_surface_b: Dictionary = poker.surface_state(run_state, run_state.current_environment, hidden_ui)
	var hidden_view_a: Dictionary = hidden_surface_a.get("double_up_view", {})
	var hidden_view_b: Dictionary = hidden_surface_b.get("double_up_view", {})
	_check(not hidden_view_a.has("picks") and not hidden_view_a.has("pick_ranks"), "Video Poker public double-up surface exposed unrevealed future picks.")
	_check(RuntimeScript.canonical_json(hidden_view_a) == RuntimeScript.canonical_json(hidden_view_b), "Video Poker public double-up observers received different sealed projections.")
	var restored = RunStateScript.new()
	restored.from_dict(run_state.to_dict())
	...
	var revisit_hidden_surface: Dictionary = revisit_poker.surface_state(restored, restored.current_environment, hidden_ui)
	var revisit_hidden_view: Dictionary = revisit_hidden_surface.get("double_up_view", {})
	_check(not revisit_hidden_view.has("picks") and not revisit_hidden_view.has("pick_ranks") and RuntimeScript.canonical_json(revisit_hidden_view) == RuntimeScript.canonical_json(hidden_view_a), "Video Poker save/revisit exposed or changed unrevealed future picks.")
