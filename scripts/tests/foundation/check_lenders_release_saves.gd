...
func _check_music_fx_foundation(library: ContentLibrary, failures: Array) -> void:
	...
	if not bool(web_contract.get("script_requires_browser_user_activation", false)):
		failures.append("Web audio unlock must verify transient browser user activation before requesting AudioContext resume.")
