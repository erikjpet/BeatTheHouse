...
func _initialize() -> void:
	...
	if int(package.get("schema_version", 0)) != 1 or str(package.get("package_id", "")) != "env06_7_queen_public": failures.append("Package E identity/version changed.")
	if str(package.get("handler_pack", "")) != "queen_public" or str(package.get("renderer_id", "")) != "queen_public": failures.append("Package E extension identity changed.")
	...
	for entry_value in _array(package.get("scenarios", [])):
		...
		var errors := Schema.validate_definition(definition, OperationRegistry, _dict(_current_host.get("target_inventory", {})))
	...
	expected.sort()
	if actual_ids != expected: failures.append("Package E exact 8-id inventory changed.")
	var catalog := Catalog.load_catalog()
	if not bool(catalog.get("ok", false)): failures.append("Catalog rejected Package E: %s" % JSON.stringify(catalog.get("failures", [])))
	for scenario_id in EXPECTED_IDS:
		var overlay := Catalog.overlay_for(scenario_id, catalog)
		if str(overlay.get("package_id", "")) != "env06_7_queen_public": failures.append("Catalog did not claim %s exactly once." % scenario_id)
...
func _initial(definition: Dictionary, seed: String) -> Dictionary:
	return Runtime.initial_state(definition, "package_e_runtime_node", seed, _host_semantics())
...
func _command(state: Dictionary, definition: Dictionary, command_id: String, receipt_id: String) -> Dictionary:
	...
	return Runtime.command(command_id,"package_e_runtime_node",str(state.get("phase_id","")),receipt_id,{},"scenario",object_id,str(descriptor.get("action_origin_owner_namespace","scenario")),str(descriptor.get("action_origin_stable_object_id",object_id)),str(descriptor.get("action_origin_receipt_key","")),str(descriptor.get("action_origin_boundary_id","")),str(descriptor.get("action_origin_fingerprint","")))

func _fact_route(state: Dictionary, definition: Dictionary, fact_type: String, payload: Dictionary, fact_id: String) -> Dictionary:
	var fact := Runtime.fact(fact_type,_producer(fact_type),"package_e_runtime_node",fact_id,1,1,payload)
	...
	if fact_type == "world_boundary" and str(_dict(flushed.get("state", {})).get("status", "")) == "active":
		var second := Runtime.fact(fact_type,_producer(fact_type),"package_e_runtime_node","%s_next" % fact_id,2,2,payload)
...
func _valid_payload(fact_type: String) -> Dictionary:
	match fact_type:
		...
		"travel_arrived": return {"source_id":"world_map","target_id":"delta_queen","travel_kind":"ordinary"}
...
func _finish(failures: Array) -> void:
	if failures.is_empty():
		print("ENV06_7_PACKAGE_E_CONTRACT_OK scenarios=8 signatures=8")
		quit(0)
	else:
