...
const ALLOWED_SEQUENCE_KEYS := [
	...
	"sequence_signature", "owner_exceptions", "fact_subscriptions", "declared_targets",
]
...
const CONDITION_KEYS := ["type", "command_id", "fact_type", "key", "value", "objective_id", "step_id", "outcome", "receipt_kind", "family", "boundary_id", "receipt_id"]
...
static func catalog_uniqueness_report(definitions: Array, expected_count: int, operation_registry: Variant = null, masked_visual_explanations: Dictionary = {}, target_inventories: Dictionary = {}) -> Dictionary:
	...
	for definition_value in definitions:
		...
		rows.append({
			"id": scenario_id,
			"signature": normalized_signature(definition),
			"authored_signature": str(sequence(definition).get("sequence_signature", "")),
			"calculated_signature": calculated_signature_hash(definition),
			"nearest_id": "", "nearest_similarity": 0.0,
		})
	...
	return {"ok": failures.is_empty(), "expected_count": expected_count, "actual_count": definitions.size(), "rows": rows, "pairs": pairs, "failures": failures, "warnings": warnings}
