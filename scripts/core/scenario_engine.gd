...
static func initial_state(definition: Dictionary) -> Dictionary:
	...
	var result := {
		...
		"mutations": JsonCoerceScript._copy_dict(definition.get("mutations", {})),
		"phases": JsonCoerceScript._copy_array(definition.get("phases", [])),
		"town_weight_tags": JsonCoerceScript._unique_string_array(definition.get("town_weight_tags", [])),
	}
...
static func normalize_state(value: Variant) -> Dictionary:
	...
	if scenario_id.is_empty():
		return {}
	var phases := JsonCoerceScript._copy_array(source.get("phases", []))
	...
	var result := {
		...
		"mutations": JsonCoerceScript._copy_dict(source.get("mutations", {})),
		"phases": phases,
		"town_weight_tags": JsonCoerceScript._unique_string_array(source.get("town_weight_tags", [])),
	}
...
static func apply_to_archetype(archetype: Dictionary, state_value: Variant) -> Dictionary:
	...
	if not _state_targets_layer(state, result):
		return result
	_apply_mutations(result, JsonCoerceScript._copy_dict(state.get("mutations", {})), true)
	var phases := JsonCoerceScript._copy_array(state.get("phases", []))
	...
	for index in range(phase_index + 1):
		if index < phases.size() and typeof(phases[index]) == TYPE_DICTIONARY:
			_apply_mutations(result, JsonCoerceScript._copy_dict((phases[index] as Dictionary).get("mutations", {})), true)
	return result
...
static func sequence_definition_for_environment(environment: Dictionary, preferred: Dictionary = {}) -> Dictionary:
	...
	if _sequence_is_suppressed(environment, preferred):
		var suppressed_source := preferred if not preferred.is_empty() and str(preferred.get("id", preferred.get("scenario_id", ""))).strip_edges() == scenario_id else {"id": scenario_id, "archetype_id": str(JsonCoerceScript._copy_dict(environment.get("scenario_state", {})).get("archetype_id", environment.get("archetype_id", "")))}
...
static func ensure_sequence_state(environment: Dictionary, definition: Dictionary, seed_token: String = "", definition_prevalidated: bool = false) -> Dictionary:
	...
	if typeof(stored_state_value) == TYPE_DICTIONARY \
		...
		var trusted_semantic := JsonCoerceScript._copy_dict(trusted_state.get("semantic_state", {}))
		...
		if str(state.get("node_id", "")) != node_id:
			if _sequence_state_can_bind_initial_node(state, environment, definition, host_semantics):
				...
				if bool(quarantine_cleanup.get("ok", false)):
					state = JsonCoerceScript._copy_dict(quarantine_cleanup.get("state", state))
				...
				if not bool(quarantine_cleanup.get("ok", false)):
					state["errors"].append_array(JsonCoerceScript._copy_array(quarantine_cleanup.get("errors", [])))
		if not node_binding_failed and not JsonCoerceScript._copy_array(host_semantics.get("inventory_errors", [])).is_empty():
			state["status"] = SequenceRuntimeScript.STATUS_CLEANED
			state["errors"] = JsonCoerceScript._copy_array(host_semantics.get("inventory_errors", []))
		elif not node_binding_failed and (int(JsonCoerceScript._copy_dict(state.get("semantic_state", {})).get("inventory_schema_version", 0)) != int(host_semantics.get("inventory_schema_version", 0)) or str(JsonCoerceScript._copy_dict(state.get("semantic_state", {})).get("inventory_digest", "")) != str(host_semantics.get("inventory_digest", ""))):
			state["status"] = SequenceRuntimeScript.STATUS_CLEANED
			...
			var target_inventory := JsonCoerceScript._copy_dict(host_semantics.get("target_inventory", {}))
			target_inventory["event_choices"] = JsonCoerceScript._copy_dict(host_semantics.get("event_choices", {}))
			...
			semantic["base_interactions"] = JsonCoerceScript._copy_array(host_semantics.get("base_interactions", []))
			semantic["event_choices"] = JsonCoerceScript._copy_dict(host_semantics.get("event_choices", {}))
			...
			if not bool(rebuilt.get("ok", false)):
				state["status"] = SequenceRuntimeScript.STATUS_CLEANED
				state["errors"] = JsonCoerceScript._copy_array(rebuilt.get("errors", []))
			else:
				state = JsonCoerceScript._copy_dict(rebuilt.get("state", state))
...
static func _rebuild_receipted_semantic_mutations(state_value: Dictionary, definition: Dictionary, host_semantics: Dictionary) -> Dictionary:
	...
	var errors: Array = []
	var causal_record_total := JsonCoerceScript._copy_array(state.get("command_receipt_records", [])).size() + JsonCoerceScript._copy_array(state.get("fact_receipt_records", [])).size() + JsonCoerceScript._copy_array(state.get("visit_receipt_records", [])).size() + JsonCoerceScript._copy_array(state.get("expiry_boundary_records", [])).size()
	if causal_record_total > SequenceRuntimeScript.MAX_RECEIPTS or causal_record_total + JsonCoerceScript._copy_array(state.get("fact_queue", [])).size() > SequenceRuntimeScript.MAX_RECEIPTS:
	...
	errors.append_array(JsonCoerceScript._copy_array(cause_validation.get("errors", [])))
	var expiry_validation := _validated_expiry_boundary_records(state, definition)
	errors.append_array(JsonCoerceScript._copy_array(expiry_validation.get("errors", [])))
	var visit_validation := _validated_visit_records(state)
	errors.append_array(JsonCoerceScript._copy_array(visit_validation.get("errors", [])))
	var delivery_validation := _validated_event_request_deliveries(state)
	errors.append_array(JsonCoerceScript._copy_array(delivery_validation.get("errors", [])))
	if not errors.is_empty(): return {"ok": false, "errors": errors}
	var causal_records := JsonCoerceScript._copy_array(cause_validation.get("ordered_records", []))
	causal_records.append_array(JsonCoerceScript._copy_array(expiry_validation.get("ordered_records", [])))
	causal_records.append_array(JsonCoerceScript._copy_array(visit_validation.get("ordered_records", [])))
	causal_records.sort_custom(func(a: Variant, b: Variant) -> bool: return int(JsonCoerceScript._copy_dict(a).get("cause_ordinal", -1)) < int(JsonCoerceScript._copy_dict(b).get("cause_ordinal", -1)))
	for record_index in range(causal_records.size()):
		if int(JsonCoerceScript._copy_dict(causal_records[record_index]).get("cause_ordinal", -1)) != record_index:
	...
	var replay_result := _replay_causal_records(replay, definition, causal_records, JsonCoerceScript._copy_array(cause_validation.get("fact_batch_records", [])), JsonCoerceScript._copy_array(delivery_validation.get("history", [])), JsonCoerceScript._copy_array(delivery_validation.get("receipts", [])))
	if not bool(replay_result.get("ok", false)):
		return {"ok": false, "errors": JsonCoerceScript._copy_array(replay_result.get("errors", []))}
	replay = JsonCoerceScript._copy_dict(replay_result.get("state", replay))
	...
	replay["fact_queue"] = JsonCoerceScript._copy_array(state.get("fact_queue", []))
	replay["fact_serial_next"] = maxi(int(replay.get("fact_serial_next", 1)), int(state.get("fact_serial_next", 1)))
	var semantic := JsonCoerceScript._copy_dict(replay.get("semantic_state", {}))
...
static func _validated_event_request_deliveries(state: Dictionary) -> Dictionary:
	...
	var receipts := JsonCoerceScript._copy_array(state.get("event_request_delivery_receipts", []))
	var seen: Dictionary = {}
...
static func _validated_cause_records(state: Dictionary, definition: Dictionary, host_semantics: Dictionary) -> Dictionary:
	...
	var command_receipts := JsonCoerceScript._copy_array(state.get("command_receipts", []))
	var command_fingerprints := JsonCoerceScript._copy_dict(state.get("command_fingerprints", {}))
	var command_records := JsonCoerceScript._copy_array(state.get("command_receipt_records", []))
	for record_index in range(command_records.size()):
		var record := JsonCoerceScript._copy_dict(command_records[record_index])
		var envelope := JsonCoerceScript._copy_dict(record.get("envelope", {}))
		var causal_descriptor := JsonCoerceScript._copy_dict(record.get("causal_action_descriptor", {}))
		if record.size() != 6 or not _exact_string_fields(record, ["receipt_key", "fingerprint", "causal_action_descriptor_fingerprint"]) or not JsonCoerceScript._valid_sha256(str(record.get("fingerprint", ""))) or not JsonCoerceScript._valid_sha256(str(record.get("causal_action_descriptor_fingerprint", ""))) or typeof(record.get("cause_ordinal")) != TYPE_INT or int(record.get("cause_ordinal", -1)) < 0 or envelope.size() != 13 or typeof(envelope.get("schema_version")) != TYPE_INT or int(envelope.get("schema_version", 0)) != SequenceRuntimeScript.COMMAND_SCHEMA_VERSION or typeof(envelope.get("payload")) != TYPE_DICTIONARY or causal_descriptor.is_empty():
	...
	var fact_receipts := JsonCoerceScript._copy_array(state.get("fact_receipts", []))
	var fact_fingerprints := JsonCoerceScript._copy_dict(state.get("fact_fingerprints", {}))
	var fact_records := JsonCoerceScript._copy_array(state.get("fact_receipt_records", []))
	var fact_validation_state := state.duplicate(true)
	fact_validation_state["status"] = SequenceRuntimeScript.STATUS_ACTIVE
	var trusted_semantic := JsonCoerceScript._copy_dict(fact_validation_state.get("semantic_state", {}))
	trusted_semantic["event_choices"] = JsonCoerceScript._copy_dict(host_semantics.get("event_choices", {}))
	...
	var batch_records := JsonCoerceScript._copy_array(state.get("fact_flush_batch_records", []))
	...
	for batch_index in range(batch_records.size()):
		var batch := JsonCoerceScript._copy_dict(batch_records[batch_index])
		var receipt_keys := JsonCoerceScript._copy_array(batch.get("fact_receipt_keys", []))
		var batch_fingerprints := JsonCoerceScript._copy_array(batch.get("fact_fingerprints", []))
		if batch.size() != 8 or typeof(batch.get("batch_ordinal")) != TYPE_INT or int(batch.get("batch_ordinal", -1)) != batch_index or typeof(batch.get("requested_boundary_serial")) != TYPE_INT or int(batch.get("requested_boundary_serial", -1)) < 0 or typeof(batch.get("effective_boundary_serial")) != TYPE_INT or int(batch.get("effective_boundary_serial", -1)) < 0 or typeof(batch.get("first_cause_ordinal")) != TYPE_INT or int(batch.get("first_cause_ordinal", -1)) < 0 or typeof(batch.get("fact_receipt_keys")) != TYPE_ARRAY or typeof(batch.get("fact_fingerprints")) != TYPE_ARRAY or receipt_keys.is_empty() or receipt_keys.size() != batch_fingerprints.size() or not _exact_string_fields(batch, ["prior_batch_fingerprint", "batch_fingerprint"]):
			...
			continue
		if str(batch.get("prior_batch_fingerprint", "")) != prior_batch_fingerprint or not JsonCoerceScript._valid_sha256(str(batch.get("batch_fingerprint", ""))) or str(batch.get("batch_fingerprint", "")) != SequenceRuntimeScript.fact_flush_batch_fingerprint(batch) or int(batch.get("effective_boundary_serial", -1)) != maxi(prior_effective_boundary, int(batch.get("requested_boundary_serial", -1))):
		...
		for batch_fact_index in range(receipt_keys.size()):
			if next_fact_record_index >= validated_fact_records.size():
				batch_valid = false
				break
			var fact_record := JsonCoerceScript._copy_dict(validated_fact_records[next_fact_record_index])
...
static func _validated_visit_records(state: Dictionary) -> Dictionary:
	var errors: Array = []
	var ordered_records: Array = []
	var receipts := JsonCoerceScript._copy_array(state.get("visit_receipts", []))
	var records := JsonCoerceScript._copy_array(state.get("visit_receipt_records", []))
	var seen: Dictionary = {}
	for record_index in range(records.size()):
		var record := JsonCoerceScript._copy_dict(records[record_index])
...
static func _validated_expiry_boundary_records(state: Dictionary, definition: Dictionary) -> Dictionary:
	var errors: Array = []
	var ordered_records: Array = []
	var authored_boundary := str(JsonCoerceScript._copy_dict(SequenceSchemaScript.sequence(definition).get("expiry", {})).get("boundary", "none"))
	for record_value in JsonCoerceScript._copy_array(state.get("expiry_boundary_records", [])):
...
static func _replay_causal_records(initial: Dictionary, definition: Dictionary, causal_records: Array, fact_batch_records: Array, delivered_history: Array = [], delivery_receipts: Array = []) -> Dictionary:
	...
	if not bool(initial_delivery_restore.get("ok", false)):
		return {"ok": false, "state": initial, "errors": JsonCoerceScript._copy_array(initial_delivery_restore.get("errors", []))}
	state = JsonCoerceScript._copy_dict(initial_delivery_restore.get("state", state))
	var record_index := 0
	var expected_fact_batch_ordinal := 0
	while record_index < causal_records.size():
		var causal := JsonCoerceScript._copy_dict(causal_records[record_index])
		var kind := str(causal.get("kind", ""))
		if kind == "fact":
			var first_fact_record := JsonCoerceScript._copy_dict(JsonCoerceScript._copy_dict(causal_records[record_index]).get("record", {}))
			var batch_ordinal := int(first_fact_record.get("flush_batch_ordinal", -1))
			var batch := JsonCoerceScript._copy_dict(fact_batch_records[expected_fact_batch_ordinal] if expected_fact_batch_ordinal < fact_batch_records.size() else {})
			var expected_receipt_keys := JsonCoerceScript._copy_array(batch.get("fact_receipt_keys", []))
			...
			while record_index < causal_records.size() and str(JsonCoerceScript._copy_dict(causal_records[record_index]).get("kind", "")) == "fact" and int(JsonCoerceScript._copy_dict(JsonCoerceScript._copy_dict(causal_records[record_index]).get("record", {})).get("flush_batch_ordinal", -1)) == batch_ordinal:
				var fact_record := JsonCoerceScript._copy_dict(JsonCoerceScript._copy_dict(causal_records[record_index]).get("record", {}))
				...
				var envelope := JsonCoerceScript._copy_dict(fact_record.get("envelope", {}))
				...
				state = JsonCoerceScript._copy_dict(queued.get("state", state))
				record_index += 1
				batch_fact_index += 1
			...
			state = JsonCoerceScript._copy_dict(flushed.get("state", state))
			...
			if not bool(fact_delivery_restore.get("ok", false)):
				return {"ok": false, "state": initial, "errors": JsonCoerceScript._copy_array(fact_delivery_restore.get("errors", []))}
			state = JsonCoerceScript._copy_dict(fact_delivery_restore.get("state", state))
			var replayed_batches := JsonCoerceScript._copy_array(state.get("fact_flush_batch_records", []))
			...
			continue
		var record := JsonCoerceScript._copy_dict(causal.get("record", {}))
		var result: Dictionary = {}
		if kind == "command":
			result = SequenceRuntimeScript.apply_command(state, definition, JsonCoerceScript._copy_dict(record.get("envelope", {})), {"available_funds": 9223372036854775807, "causal_action_descriptor": JsonCoerceScript._copy_dict(record.get("causal_action_descriptor", {}))})
		elif kind == "visit":
		...
		state = JsonCoerceScript._copy_dict(result.get("state", state))
		...
		if not bool(delivery_restore.get("ok", false)):
			return {"ok": false, "state": initial, "errors": JsonCoerceScript._copy_array(delivery_restore.get("errors", []))}
		state = JsonCoerceScript._copy_dict(delivery_restore.get("state", state))
		record_index += 1
...
static func _restore_replayed_event_request_deliveries(state_value: Dictionary, delivered_history: Array, delivery_receipts: Array) -> Dictionary:
	var state := state_value.duplicate(true)
	var restored_history := JsonCoerceScript._copy_array(state.get("event_request_history", []))
	var restored_receipts := JsonCoerceScript._copy_array(state.get("event_request_delivery_receipts", []))
	...
	var queue := JsonCoerceScript._copy_array(state.get("event_request_queue", []))
	while restored_history.size() < delivered_history.size():
		var index := restored_history.size()
		var expected := JsonCoerceScript._copy_dict(delivered_history[index])
		var expected_id := str(delivery_receipts[index]) if index < delivery_receipts.size() else ""
		var queue_index := -1
		for candidate_index in range(queue.size()):
			if str(JsonCoerceScript._copy_dict(queue[candidate_index]).get("request_id", "")) == expected_id:
				queue_index = candidate_index
				break
		if queue_index < 0:
			break
		var replayed_request := JsonCoerceScript._copy_dict(queue[queue_index])
...
static func _exact_replay_journal_errors(stored: Dictionary, replayed: Dictionary) -> Array:
	...
	var stored_semantic := JsonCoerceScript._copy_dict(stored.get("semantic_state", {}))
	var replayed_semantic := JsonCoerceScript._copy_dict(replayed.get("semantic_state", {}))
...
static func _validated_branch_resolution_records(state: Dictionary, definition: Dictionary, transition_records: Array, causes: Dictionary) -> Dictionary:
	...
	var current_phase := str(JsonCoerceScript._copy_dict(transition_records[0] if not transition_records.is_empty() else {}).get("phase_id", ""))
	var terminal_seen := false
	var records := JsonCoerceScript._copy_array(state.get("branch_resolution_records", []))
	for record_index in range(records.size()):
		var record := JsonCoerceScript._copy_dict(records[record_index])
		...
		if record.size() != 8 or has_target == has_outcome or not _exact_string_fields(record, string_fields) or typeof(record.get("boundary_ordinal")) != TYPE_INT or int(record.get("boundary_ordinal", -1)) != record_index:
			...
			continue
		while current_phase != str(record.get("phase_id", "")) and transition_index + 1 < transition_records.size() and str(JsonCoerceScript._copy_dict(transition_records[transition_index + 1]).get("source_receipt", "")) == "ordered_compat":
			transition_index += 1
			current_phase = str(JsonCoerceScript._copy_dict(transition_records[transition_index]).get("phase_id", ""))
		...
		var cause := JsonCoerceScript._copy_dict(causes.get(cause_key, {}))
		...
		if terminal_seen or current_phase != phase_id or authored_branch.is_empty() or resolved_branch_ids.has(resolved_id) or used_causes.has(cause_key) or cause.is_empty() or str(cause.get("fingerprint", "")) != str(record.get("cause_fingerprint", "")) or str(record.get("branch_fingerprint", "")) != SequenceRuntimeScript.branch_content_fingerprint(authored_branch):
			...
			continue
		var envelope := JsonCoerceScript._copy_dict(cause.get("envelope", {}))
		...
		for candidate_value in JsonCoerceScript._copy_array(SequenceSchemaScript.phase(definition, phase_id).get("branches", [])):
			var candidate := JsonCoerceScript._copy_dict(candidate_value)
			if _branch_condition_matches_cause(JsonCoerceScript._copy_dict(candidate.get("condition", {})), trigger_kind, envelope, state, resolved_outcomes, causes, definition, phase_id):
		...
		if has_target:
			...
			if str(authored_branch.get("next_phase", "")) != target_phase or transition_index + 1 >= transition_records.size():
				...
				continue
			var next_transition := JsonCoerceScript._copy_dict(transition_records[transition_index + 1])
			...
			if str(authored_branch.get("outcome", "")) != outcome_id or JsonCoerceScript._copy_dict(JsonCoerceScript._copy_dict(SequenceSchemaScript.sequence(definition).get("aftermath", {})).get(outcome_id, {})).is_empty():
	...
	for index in range(1, transition_records.size()):
		var source_receipt := str(JsonCoerceScript._copy_dict(transition_records[index]).get("source_receipt", ""))
...
static func _authored_branch(definition: Dictionary, phase_id: String, branch_id: String) -> Dictionary:
	for branch_value in JsonCoerceScript._copy_array(SequenceSchemaScript.phase(definition, phase_id).get("branches", [])):
		var branch := JsonCoerceScript._copy_dict(branch_value)
...
static func _branch_condition_matches_cause(condition: Dictionary, trigger_kind: String, envelope: Dictionary, state: Dictionary, proven_outcomes: Array, causes: Dictionary, definition: Dictionary, phase_id: String) -> bool:
	...
	var inputs := JsonCoerceScript._copy_dict(handler.get("inputs", {}))
	match str(condition.get("type", "")):
		...
		"local_equals":
			if handler_id == "set_local" and str(inputs.get("key", "")) == str(condition.get("key", "")):
				...
				var value: Variant = JsonCoerceScript._copy_dict(envelope.get("payload", {})).get(projected_key) if not projected_key.is_empty() else inputs.get("value")
				return value == condition.get("value")
			return JsonCoerceScript._copy_dict(state.get("local_state", {})).get(str(condition.get("key", ""))) == condition.get("value")
		"local_min":
			var local_value: Variant = JsonCoerceScript._copy_dict(state.get("local_state", {})).get(str(condition.get("key", "")))
		...
		"objective":
			...
			return JsonCoerceScript._copy_array(JsonCoerceScript._copy_dict(JsonCoerceScript._copy_dict(state.get("objective_progress", {})).get(str(condition.get("objective_id", "")), {})).get("completed_steps", [])).has(str(condition.get("step_id", "")))
...
static func _causal_recorded_outcomes(definition: Dictionary, cause_validation: Dictionary) -> Array:
	...
	for record_key in ["command_records", "fact_records"]:
		for record_value in JsonCoerceScript._copy_array(cause_validation.get(record_key, [])):
			var envelope := JsonCoerceScript._copy_dict(JsonCoerceScript._copy_dict(record_value).get("envelope", {}))
			...
			if str(handler.get("handler", "")) == "record_outcome": _append_unique_text(result, str(JsonCoerceScript._copy_dict(handler.get("inputs", {})).get("outcome", "")))
	return result
...
static func _authored_handler_for_cause(definition: Dictionary, trigger_kind: String, envelope: Dictionary, phase_id: String) -> Dictionary:
	if trigger_kind == "fact":
		for subscription_value in JsonCoerceScript._copy_array(SequenceSchemaScript.sequence(definition).get("fact_subscriptions", [])):
	...
	for operation_value in JsonCoerceScript._copy_array(SequenceSchemaScript.phase(definition, phase_id).get("interaction_ops", [])):
		var operation := JsonCoerceScript._copy_dict(operation_value)
		var interaction := JsonCoerceScript._copy_dict(operation.get("interaction", {}))
		for action_value in JsonCoerceScript._copy_array(interaction.get("available_actions", operation.get("available_actions", []))):
...
static func _command_completes_objective_step(definition: Dictionary, command_id: String, objective_id: String, step_id: String) -> bool:
	for objective_value in JsonCoerceScript._copy_array(SequenceSchemaScript.sequence(definition).get("objectives", [])):
		var objective := JsonCoerceScript._copy_dict(objective_value)
		if str(objective.get("id", "")) != objective_id: continue
		for step_value in JsonCoerceScript._copy_array(objective.get("steps", [])):
			var step := JsonCoerceScript._copy_dict(step_value)
...
static func _journal_receipt_condition_proven(condition: Dictionary, state: Dictionary, causes: Dictionary) -> bool:
	...
	var records := JsonCoerceScript._copy_array(JsonCoerceScript._copy_dict(state.get("semantic_state", {})).get(collection_key, [])) if receipt_kind == "operation" else JsonCoerceScript._copy_array(state.get(collection_key, []))
...
static func _register_allowed_operations(allowed: Dictionary, family_override: String, operations: Array, boundary_id: String, cleanup_restore: bool, errors: Array) -> void:
	...
	for proof_value in allowed.values():
		if str(JsonCoerceScript._copy_dict(proof_value).get("boundary_id", "")) == boundary_id: boundary_ordinal += 1
...
static func _valid_phase_transition(definition: Dictionary, prior_phase: String, phase_id: String, source_receipt: String) -> bool:
	...
	for branch_value in JsonCoerceScript._copy_array(SequenceSchemaScript.phase(definition, prior_phase).get("branches", [])):
		var branch := JsonCoerceScript._copy_dict(branch_value)
...
static func _authored_cleanup_reasons(definition: Dictionary) -> Array:
	...
	for subscription_value in JsonCoerceScript._copy_array(sequence.get("fact_subscriptions", [])):
		var subscription := JsonCoerceScript._copy_dict(subscription_value)
		if str(subscription.get("handler", "")) == "request_cleanup": _append_unique_text(result, str(JsonCoerceScript._copy_dict(subscription.get("inputs", {})).get("reason", "")))
	for phase_value in JsonCoerceScript._copy_array(JsonCoerceScript._copy_dict(sequence.get("phase_graph", {})).get("phases", [])):
		for operation_value in JsonCoerceScript._copy_array(JsonCoerceScript._copy_dict(phase_value).get("interaction_ops", [])):
			var operation := JsonCoerceScript._copy_dict(operation_value)
			var interaction := JsonCoerceScript._copy_dict(operation.get("interaction", {}))
			for action_value in JsonCoerceScript._copy_array(interaction.get("available_actions", operation.get("available_actions", []))):
				var action := JsonCoerceScript._copy_dict(action_value)
				if str(action.get("handler", "")) == "request_cleanup": _append_unique_text(result, str(JsonCoerceScript._copy_dict(action.get("inputs", {})).get("reason", "")))
	return result


static func _all_branches(definition: Dictionary) -> Array:
	var result: Array = []
	for phase_value in JsonCoerceScript._copy_array(JsonCoerceScript._copy_dict(SequenceSchemaScript.sequence(definition).get("phase_graph", {})).get("phases", [])):
		var phase := JsonCoerceScript._copy_dict(phase_value)
		for branch_value in JsonCoerceScript._copy_array(phase.get("branches", [])):
			var branch := JsonCoerceScript._copy_dict(branch_value)
...
static func _record_keys(records: Array) -> Array:
	var result: Array = []
	for record_value in records: result.append(str(JsonCoerceScript._copy_dict(record_value).get("receipt_key", "")))
	return result
...
static func sequence_host_semantics(environment: Dictionary) -> Dictionary:
	var base_interactions := JsonCoerceScript._copy_array(environment.get("scenario_base_interactions", []))
	var sealed_inventory := JsonCoerceScript._copy_dict(environment.get("scenario_semantic_inventory", {}))
	...
	if typeof(environment.get("scenario_semantic_action_digest")) != TYPE_STRING or not JsonCoerceScript._valid_sha256(str(environment.get("scenario_semantic_action_digest", ""))) or str(environment.get("scenario_semantic_action_digest", "")) != SequenceRuntimeScript.base_interaction_action_authority_digest(base_interactions):
	...
	return {"target_inventory": EnvironmentSemanticInventoryScript.exact_collections(sealed_inventory), "inventory_schema_version": int(sealed_inventory.get("schema_version", 0)), "inventory_digest": str(sealed_inventory.get("digest", "")), "inventory_errors": [], "base_interactions": base_interactions, "event_choices": JsonCoerceScript._copy_dict(environment.get("scenario_event_choices", {}))}
...
static func _trusted_host_semantics_match(environment: Dictionary, trusted_semantic: Dictionary) -> bool:
	...
	return JsonCoerceScript._valid_sha256(action_digest) \
...
static func sequence_command(environment: Dictionary, definition: Dictionary, command: Dictionary, context: Dictionary = {}) -> Dictionary:
	...
	if not bool(dispatched.get("ok", false)):
		return {"ok": false, "errors": JsonCoerceScript._copy_array(dispatched.get("errors", [])), "state": state}
	var result := SequenceRuntimeScript.apply_command(state, definition, JsonCoerceScript._copy_dict(dispatched.get("command", command)), JsonCoerceScript._copy_dict(dispatched.get("context", context)), true)
...
static func _node_binding_quarantine_result(state: Dictionary) -> Dictionary:
	...
	if str(state.get("status", "")) != SequenceRuntimeScript.STATUS_CLEANED or not JsonCoerceScript._copy_array(state.get("errors", [])).has(binding_error):
		return {}
	return {"ok": false, "quarantine_kind": "node_binding", "errors": JsonCoerceScript._copy_array(state.get("errors", [])), "state": state.duplicate(true)}
...
static func sequence_projection(environment: Dictionary, definition: Dictionary = {}) -> Dictionary:
	...
	var host_semantics := sequence_host_semantics(environment)
	if not JsonCoerceScript._copy_array(host_semantics.get("inventory_errors", [])).is_empty(): return {}
	var state := SequenceRuntimeScript.normalize_state(environment.get("scenario_sequence_state", {}), definition, host_semantics)
...
static func drain_sequence_transitions(environment: Dictionary, definition: Dictionary, reduced_motion: bool = false) -> Dictionary:
	...
	definition = sequence_definition_for_environment(environment, definition)
	var stored_state := JsonCoerceScript._copy_dict(environment.get("scenario_sequence_state", {}))
	var stored_semantic := JsonCoerceScript._copy_dict(stored_state.get("semantic_state", {}))
	if JsonCoerceScript._copy_array(stored_semantic.get("transition_queue", [])).is_empty():
...
static func drain_sequence_event_requests(environment: Dictionary, definition: Dictionary) -> Dictionary:
	...
	definition = sequence_definition_for_environment(environment, definition)
	var stored_state := JsonCoerceScript._copy_dict(environment.get("scenario_sequence_state", {}))
	if JsonCoerceScript._copy_array(stored_state.get("event_request_queue", [])).is_empty():
...
static func _commit_sequence_drain(environment: Dictionary, definition: Dictionary, result_value: Dictionary) -> Dictionary:
	...
	if not bool(result.get("ok", false)):
		return result
	var next := JsonCoerceScript._copy_dict(result.get("state", {}))
	var current_projection := JsonCoerceScript._copy_dict(environment.get("scenario_sequence_projection", {}))
	var render_snapshot := JsonCoerceScript._copy_dict(environment.get("scenario_render_snapshot", {}))
	...
	render_snapshot["active_stages"] = JsonCoerceScript._copy_array(fresh_projection.get("active_stages", []))
	...
	result["layout_authority"] = JsonCoerceScript._copy_dict(environment.get("scenario_layout_authority", {}))
	result["layout_authority_digest"] = str(environment.get("scenario_layout_authority_digest", ""))
	result["layout_audit"] = JsonCoerceScript._copy_dict(environment.get("scenario_layout_audit", {}))
...
static func _sequence_is_suppressed(environment: Dictionary, preferred: Dictionary = {}) -> bool:
	return bool(JsonCoerceScript._copy_dict(environment.get("scenario_state", {})).get(SEQUENCE_SUPPRESSION_KEY, false)) or bool(preferred.get(SEQUENCE_SUPPRESSION_KEY, false))
...
static func _clear_environment_sequence(environment: Dictionary) -> void:
	...
	for baseline_key_value in baseline_arrays.keys():
		...
		if environment.has(baseline_key):
			environment[str(baseline_arrays.get(baseline_key_value, ""))] = JsonCoerceScript._copy_array(environment.get(baseline_key, []))
	if environment.has("scenario_sequence_base_game_modifiers"):
		environment["scenario_game_modifiers"] = JsonCoerceScript._copy_dict(environment.get("scenario_sequence_base_game_modifiers", {}))
	for key in [
...
static func _refresh_sequence_snapshots(environment: Dictionary, definition: Dictionary) -> Dictionary:
	...
	if not bool(committed.get("ok", false)):
		return {"ok": false, "errors": JsonCoerceScript._copy_array(committed.get("errors", []))}
	return JsonCoerceScript._copy_dict(environment.get("scenario_render_snapshot", {}))
...
static func _commit_sequence_candidate(environment: Dictionary, candidate_value: Dictionary, definition: Dictionary, result_value: Dictionary) -> Dictionary:
	...
	var layout_context := JsonCoerceScript._copy_dict(candidate.get("scenario_layout_context", {}))
	if not layout_context.is_empty():
		layout_environment["_scenario_layout_context"] = layout_context
	var layout_result := ScenarioLayoutResolverScript.resolve(JsonCoerceScript._copy_array(candidate.get("scenario_layout_base_records", [])), projection, layout_environment)
	if not bool(layout_result.get("ok", false)):
		return _sequence_candidate_failure(environment, result, JsonCoerceScript._copy_array(layout_result.get("errors", ["Scenario post-operation layout validation failed closed."])))
	var sealed_projection := JsonCoerceScript._copy_dict(layout_result.get("projection", {}))
	...
	if not bool(renderer_snapshot.get("ok", false)):
		return _sequence_candidate_failure(environment, result, JsonCoerceScript._copy_array(renderer_snapshot.get("errors", ["Scenario post-operation renderer validation failed closed."])))
	...
	result["layout_authority"] = JsonCoerceScript._copy_dict(layout_result.get("layout_authority", {}))
	result["layout_authority_digest"] = authority_digest
	result["layout_audit"] = JsonCoerceScript._copy_dict(layout_result.get("layout_audit", {}))
	result["renderer_snapshot"] = renderer_snapshot.duplicate(true)
	result["warnings"] = JsonCoerceScript._copy_array(layout_result.get("warnings", []))
	return result
...
static func _commit_reused_sequence_layout(environment: Dictionary, candidate: Dictionary, next: Dictionary, projection: Dictionary, result_value: Dictionary, input_digest: String) -> Dictionary:
	...
	if typeof(stored_input_value) != TYPE_STRING \
		or not JsonCoerceScript._valid_sha256(str(stored_input_value)) \
		or str(stored_input_value) != input_digest:
		return {}
	var stored_projection := JsonCoerceScript._copy_dict(candidate.get("scenario_sequence_projection", {}))
	var authority := JsonCoerceScript._copy_dict(candidate.get("scenario_layout_authority", {}))
	var audit := JsonCoerceScript._copy_dict(candidate.get("scenario_layout_audit", {}))
	...
	if stored_projection.is_empty() or not bool(audit.get("active", false)) or not bool(audit.get("valid", false)) \
		or not JsonCoerceScript._valid_sha256(authority_digest) \
		or str(audit.get("authority_digest", "")) != authority_digest:
		return {}
	var sealed_projection := projection.duplicate(true)
	sealed_projection["semantic_state"] = JsonCoerceScript._copy_dict(stored_projection.get("semantic_state", {}))
...
static func _commit_passive_sequence_candidate(environment: Dictionary, candidate: Dictionary, next: Dictionary, projection_value: Dictionary, result_value: Dictionary) -> Dictionary:
	...
	if bool(audit.get("active", true)):
		return {}
	var renderer := JsonCoerceScript._copy_dict(candidate.get("scenario_render_snapshot", {}))
	var authority := JsonCoerceScript._copy_dict(candidate.get("scenario_layout_authority", {}))
	var authority_digest := str(candidate.get("scenario_layout_authority_digest", ""))
	...
	var semantic := JsonCoerceScript._copy_dict(projection.get("semantic_state", {}))
...
static func _sequence_candidate_failure(environment: Dictionary, result_value: Dictionary, errors_value: Array) -> Dictionary:
	...
	result["state"] = JsonCoerceScript._copy_dict(environment.get("scenario_sequence_state", {}))
...
static func _capture_sequence_baseline(environment: Dictionary) -> void:
	if not environment.has("scenario_sequence_base_game_ids"):
		environment["scenario_sequence_base_game_ids"] = JsonCoerceScript._unique_string_array(environment.get("game_ids", []))
	if not environment.has("scenario_sequence_base_service_ids"):
		environment["scenario_sequence_base_service_ids"] = JsonCoerceScript._unique_string_array(environment.get("service_ids", []))
	if not environment.has("scenario_sequence_base_travel_hooks"):
		environment["scenario_sequence_base_travel_hooks"] = JsonCoerceScript._unique_string_array(environment.get("travel_hooks", []))
	if not environment.has("scenario_sequence_base_game_modifiers"):
		environment["scenario_sequence_base_game_modifiers"] = JsonCoerceScript._copy_dict(environment.get("scenario_game_modifiers", {}))
	if not environment.has("scenario_sequence_base_layout_object_rects"):
		environment["scenario_sequence_base_layout_object_rects"] = JsonCoerceScript._copy_dict(JsonCoerceScript._copy_dict(environment.get("layout", {})).get("object_rects", {}))


static func _materialize_sequence_services_games_routes(environment: Dictionary, projection: Dictionary) -> void:
	_capture_sequence_baseline(environment)
	var semantic := JsonCoerceScript._copy_dict(projection.get("semantic_state", {}))
	var services := JsonCoerceScript._unique_string_array(environment.get("scenario_sequence_base_service_ids", []))
	for value in JsonCoerceScript._copy_dict(semantic.get("services", {})).values():
		var record := JsonCoerceScript._copy_dict(value)
	...
	var games := JsonCoerceScript._unique_string_array(environment.get("scenario_sequence_base_game_ids", []))
	var modifiers := JsonCoerceScript._copy_dict(environment.get("scenario_sequence_base_game_modifiers", {}))
	for value in JsonCoerceScript._copy_dict(semantic.get("games", {})).values():
		var record := JsonCoerceScript._copy_dict(value)
		...
		if bool(record.get("enabled", true)):
			...
			games.erase(game_id)
		var modifier := JsonCoerceScript._copy_dict(record.get("modifier", {}))
	...
	var routes := JsonCoerceScript._unique_string_array(environment.get("scenario_sequence_base_travel_hooks", []))
	for value in JsonCoerceScript._copy_dict(semantic.get("routes", {})).values():
		var record := JsonCoerceScript._copy_dict(value)
...
static func validate_sequence_definition(definition: Dictionary, references: Dictionary = {}, target_inventory: Dictionary = {}) -> Array:
	...
	if not JsonCoerceScript._copy_dict(references.get("archetype_ids", {})).has(archetype_id):
	...
	if str(authoring.get("arrival_summary", "")).strip_edges().is_empty() or JsonCoerceScript._unique_string_array(authoring.get("player_verbs", [])).is_empty() or JsonCoerceScript._unique_string_array(authoring.get("world_connections", [])).is_empty():
		errors.append("scenario %s sequence authoring requires arrival_summary, player_verbs, and world_connections." % scenario_id)
	if JsonCoerceScript._unique_string_array(authoring.get("capture_ids", [])).is_empty() or JsonCoerceScript._copy_dict(authoring.get("seed_evidence", {})).is_empty():
	...
	var archetype := JsonCoerceScript._copy_dict(references.get("archetype", {}))
	for operation_value in _sequence_operations(definition):
		...
		if owner != "scenario" and not JsonCoerceScript._unique_string_array(JsonCoerceScript._copy_dict(authoring.get("references", {})).get("objects", [])).has(identity):
		...
		for anchor_key in ["anchor_id", "zone_id"]:
			...
			if anchor.is_empty():
				var payload := JsonCoerceScript._copy_dict(operation.get("object", operation.get("actor", {})))
		...
		if str(operation.get("family", "")) == "actor_ops" and str(operation.get("op", "")) == "spawn":
			var actor_id := str(JsonCoerceScript._copy_dict(operation.get("actor", {})).get("actor_id", "")).strip_edges()
			...
			if not JsonCoerceScript._copy_dict(references.get("actor_ids", {})).has(actor_id) and str(scenario_local_actor_ids.get(local_identity, "")) != actor_id:
	...
	if errors.is_empty():
		var layout_environment := {
			...
			"semantic_zones": JsonCoerceScript._copy_dict(archetype.get("semantic_zones", {})),
			"semantic_anchors": JsonCoerceScript._copy_dict(archetype.get("semantic_anchors", {})),
			"scenario_semantic_inventory": JsonCoerceScript._copy_dict(references.get("scenario_semantic_inventory", {})),
			"travel_hooks": JsonCoerceScript._copy_array(archetype.get("travel_hooks", [])),
			"next_archetypes": JsonCoerceScript._copy_array(archetype.get("next_archetypes", [])),
		}
		var initial := SequenceRuntimeScript.initial_state(definition, str(layout_environment.get("world_node_id", "")), "content_validation", {
			...
			"event_choices": JsonCoerceScript._copy_dict(target_inventory.get("event_choices", {})),
		})
		...
		if not bool(prepared.get("ok", false)):
			for layout_error_value in JsonCoerceScript._copy_array(prepared.get("errors", [])):
...
static func validate_sequence_catalog_and_audit(definitions: Array, validation_references: Array, expected_count: int, masked_visual_explanations: Dictionary = {}, target_inventories: Dictionary = {}) -> Dictionary:
	...
	for definition_index in range(definitions.size()):
		var definition := JsonCoerceScript._copy_dict(definitions[definition_index])
		var references := JsonCoerceScript._copy_dict(validation_references[definition_index]) if definition_index < validation_references.size() else {}
		var scenario_id := str(definition.get("id", "")).strip_edges()
		var target_inventory := JsonCoerceScript._copy_dict(target_inventories.get(scenario_id, {}))
...
static func _sequence_validation_receipts_match(definitions: Array, expected_count: int, target_inventories: Dictionary, receipts: Array, registry_context_fingerprint: String) -> bool:
	...
	for definition_index in range(definitions.size()):
		...
		var expected_receipt := _sequence_validation_receipt(definition, JsonCoerceScript._copy_dict(target_inventories.get(scenario_id, {})), registry_context_fingerprint)
		var actual_receipt := JsonCoerceScript._copy_dict(receipts[definition_index])
...
static func _validate_sequence_references(scenario_id: String, authoring: Dictionary, references: Dictionary, scenario_local_actor_ids: Dictionary, errors: Array) -> void:
	var authored_refs := JsonCoerceScript._copy_dict(authoring.get("references", {}))
	var known := {
	...
	for key_value in authored_refs.keys():
		var key := str(key_value)
		if key == "objects":
			for identity_value in JsonCoerceScript._unique_string_array(authored_refs.get(key, [])):
		...
		if not known.has(key):
			...
			continue
		var valid := JsonCoerceScript._copy_dict(references.get(str(known.get(key, "")), {}))
		for id_value in JsonCoerceScript._unique_string_array(authored_refs.get(key, [])):
...
static func _sequence_operations(definition: Dictionary) -> Array:
	...
	for phase_value in JsonCoerceScript._copy_array(JsonCoerceScript._copy_dict(authored.get("phase_graph", {})).get("phases", [])):
		var phase := JsonCoerceScript._copy_dict(phase_value)
		for family in ["scene_ops", "interaction_ops", "actor_ops", "transition_ops"]:
			result.append_array(JsonCoerceScript._copy_array(phase.get(family, [])))
	result.append_array(JsonCoerceScript._copy_array(JsonCoerceScript._copy_dict(authored.get("cleanup", {})).get("operations", [])))
	for aftermath_value in JsonCoerceScript._copy_dict(authored.get("aftermath", {})).values():
		var aftermath := JsonCoerceScript._copy_dict(aftermath_value)
		for family in ["scene_ops", "interaction_ops", "actor_ops", "service_ops", "game_ops", "route_ops"]:
			result.append_array(JsonCoerceScript._copy_array(aftermath.get(family, [])))
	return result
...
static func _valid_scenario_local_actor_ids(definition: Dictionary) -> Dictionary:
	...
	for operation_value in _sequence_operations(definition):
		...
		if str(operation.get("stable_object_id", "")).strip_edges().is_empty():
			continue
		var actor := JsonCoerceScript._copy_dict(operation.get("actor", {}))
...
static func _sequence_anchor_exists(archetype: Dictionary, anchor_kind: String, anchor: String) -> bool:
	if anchor_kind == "zone_id":
		if JsonCoerceScript._copy_dict(archetype.get("semantic_zones", {})).has(anchor):
			return true
		return anchor in ["left", "right", "center", "foreground", "background", "exit_lane", "service_lane"]
	if JsonCoerceScript._copy_dict(archetype.get("semantic_anchors", {})).has(anchor):
		return true
	...
	if field.is_empty():
		return false
	return int(parts[2]) >= 0 and int(parts[2]) < JsonCoerceScript._copy_array(JsonCoerceScript._copy_dict(archetype.get("layout", {})).get(field, [])).size()
...
static func advance_environment(environment: Dictionary, amount: int) -> bool:
	...
	while remaining > 0:
		...
		if applies_here:
			_apply_mutations(environment, JsonCoerceScript._copy_dict(next_phase.get("mutations", {})), false)
...
static func reconcile_environment(environment: Dictionary, state_value: Variant) -> void:
	...
	for index in range(applied_index + 1, target_index + 1):
		if index >= 0 and index < phases.size() and typeof(phases[index]) == TYPE_DICTIONARY:
			_apply_mutations(environment, JsonCoerceScript._copy_dict((phases[index] as Dictionary).get("mutations", {})), false)
...
static func _apply_mutations(target: Dictionary, mutations: Dictionary, generation: bool) -> void:
	...
	if mutations.has("patron_set"):
		target["scenario_patron_ids"] = JsonCoerceScript._unique_string_array(mutations.get("patron_set", []))
	...
	if mutations.has("economic_profile_overrides"):
		target["economic_profile"] = _deep_merge(JsonCoerceScript._copy_dict(target.get("economic_profile", {})), JsonCoerceScript._copy_dict(mutations.get("economic_profile_overrides", {})))
	if mutations.has("game_modifier_hooks"):
		target["scenario_game_modifiers"] = _deep_merge(JsonCoerceScript._copy_dict(target.get("scenario_game_modifiers", {})), JsonCoerceScript._copy_dict(mutations.get("game_modifier_hooks", {})))
	if mutations.has("music_profile_override"):
		target["music_profile"] = _deep_merge(JsonCoerceScript._copy_dict(target.get("music_profile", {})), JsonCoerceScript._copy_dict(mutations.get("music_profile_override", {})))
	if mutations.has("presentation"):
		var presentation := _deep_merge(JsonCoerceScript._copy_dict(target.get("scenario_presentation", {})), JsonCoerceScript._copy_dict(mutations.get("presentation", {})))
		target["scenario_presentation"] = presentation
		target["visual_context"] = _deep_merge(JsonCoerceScript._copy_dict(target.get("visual_context", {})), presentation)
	if mutations.has("exclusive_opportunity"):
		target["scenario_exclusive_opportunity"] = JsonCoerceScript._copy_dict(mutations.get("exclusive_opportunity", {}))
	if mutations.has("security_overrides"):
		target["security_profile"] = _deep_merge(JsonCoerceScript._copy_dict(target.get("security_profile", {})), JsonCoerceScript._copy_dict(mutations.get("security_overrides", {})))
	if mutations.has("hook_flags"):
		target["scenario_hook_flags"] = _deep_merge(JsonCoerceScript._copy_dict(target.get("scenario_hook_flags", {})), JsonCoerceScript._copy_dict(mutations.get("hook_flags", {})))
...
static func _apply_exclusive_opportunity(environment: Dictionary) -> void:
	var opportunity := JsonCoerceScript._copy_dict(environment.get("scenario_exclusive_opportunity", {}))
...
static func _apply_id_delta(target: Dictionary, key: String, additions_value: Variant, removals_value: Variant) -> void:
	var values := JsonCoerceScript._unique_string_array(target.get(key, []))
	for remove_id in JsonCoerceScript._unique_string_array(removals_value):
		while values.has(remove_id):
			values.erase(remove_id)
	for add_id in JsonCoerceScript._unique_string_array(additions_value):
...
static func _apply_item_offer_delta(target: Dictionary, key: String, additions_value: Variant, removals_value: Variant) -> void:
	var offers := JsonCoerceScript._copy_array(target.get(key, []))
	var remove_ids := JsonCoerceScript._unique_string_array(removals_value)
	...
	for offer_value in JsonCoerceScript._copy_array(additions_value):
