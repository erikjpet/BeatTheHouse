...
def make_wordmark(width, title_px=150, sub_px=40, tagline="A SEEDABLE CASINO ROGUELIKE"):
    ...
    while tracked_width(tf, title, tr) > maxw and title_px > 12:
        ...
        tf = ImageFont.truetype(FONT_BOLD, title_px)
    sf = ImageFont.truetype(FONT_BOLD, sub_px)
    while tracked_width(sf, tagline, int(sub_px * 0.32)) > maxw and sub_px > 8:
        sub_px -= 2
        sf = ImageFont.truetype(FONT_BOLD, sub_px)
    h = int(title_px * 1.7 + sub_px * 2)
    ...
    sub, sr = tagline, int(sub_px * 0.32)
    sw = tracked_width(sf, sub, sr)
    sy = ty + title_px * 1.28
    ry = sy - 22
    d.line([(width / 2 - sw / 2 - 40, ry), (width / 2 - sw / 2 - 14, ry)], fill=PINK + (255,), width=4)
    d.line([(width / 2 + sw / 2 + 14, ry), (width / 2 + sw / 2 + 40, ry)], fill=PINK + (255,), width=4)
    draw_tracked(d, ((width - sw) / 2, sy), sub, sf, CYAN + (235,), sr)
    return layer
