...
& (Join-Path $PSScriptRoot "bootstrap_native_solver.ps1") -Target $Platform
...
if ($Platform -eq "Windows") {
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $mingw "bin/clang++.exe"))) {
        throw "Pinned LLVM-MinGW compiler is missing."
    }
}

& (Join-Path $PSScriptRoot "verify_native_solver_runtime.ps1") -GodotPath $GodotPath -RequireWebTemplate:($Platform -eq "Web")
if ($LASTEXITCODE -ne 0) {
...
foreach ($entry in [System.Environment]::GetEnvironmentVariables([System.EnvironmentVariableTarget]::Process).GetEnumerator()) {
    $previousEnvironment[[string]$entry.Key] = [string]$entry.Value
}
try {
    if ($Platform -eq "Windows") {
    ...
    foreach ($name in $previousEnvironment.Keys) {
        [System.Environment]::SetEnvironmentVariable([string]$name, [string]$previousEnvironment[$name], [System.EnvironmentVariableTarget]::Process)
    }
}

Write-Host "Native Coin Pusher $Platform $Target build complete." -ForegroundColor Green
