...
func _run() -> void:
	...
	var authority_report := ScenarioEngineScript.sequence_catalog_audit(definitions, expected_count, masked_explanations)
...
func _hostile_fixture_report(definition: Dictionary) -> Array:
	...
	result.append(_schema_rejection("unreachable_phase", unreachable, "unreachable phase"))
	...
	result.append(_schema_rejection("nonterminating_graph", nonterminating, "no path to a terminal outcome"))

	var no_cleanup := definition.duplicate(true)
	no_cleanup["sequence"]["cleanup"]["operations"] = []
	result.append(_schema_rejection("missing_cleanup", no_cleanup, "cleanup must declare operations"))
	...
	result.append(_schema_rejection("duplicate_receipt", duplicate_receipt, "duplicate authored receipt_id"))
	...
	result.append(_schema_rejection("duplicate_reward_material_effect", duplicate_reward, "duplicates the normalized material effect"))
	...
	result.append(_schema_rejection("unreadable_state_only_transition", state_only, "completion_contract.arrival_readable is not supported"))
	...
	var initial := SequenceRuntimeScript.initial_state(definition, "audit_node", "hostile_orphan_hit_region")
	var orphan_result := OperationRegistryScript.resolve_interactions([], _dict(_dict(initial.get("semantic_state", {})).get("interactions", {})).values())
