...
const outputPath = path.resolve(String(args.out ?? ".tmp/env06_6/parity_performance/web_process.json"));
...
try {
  ...
  page.on("console", (message) => {
    ...
    if (text.startsWith("ENV06_6_SEQUENCE_PROBE=")) {
      report = JSON.parse(text.slice("ENV06_6_SEQUENCE_PROBE=".length));
  ...
  if (report === null) throw new Error(`Timed out after ${timeoutMs}ms waiting for ENV06_6_SEQUENCE_PROBE.`);
  ...
  if (!report.ok || report.platform !== "Web" || !report.semantic_sha256) {
    throw new Error(`Web scenario probe failed: ${JSON.stringify(report.failures ?? [])}`);
  }
  console.log(`ENV06_6_WEB_CAPTURE PASS output=${outputPath} semantic=${report.semantic_sha256} cpu=${cpu}`);
} finally {
