param(
    [string]$ManifestPath = "tools/archive/playtest06_2/playtest06_2_candidate_seeds.json",
    [switch]$RequireFinal,
    ...
    if ($null -eq $manifest.owner_build_evidence) {
        ...
        if ($null -ne $committedBuild) {
            ...
            if ($null -ne $ownerBuild) {
                ...
                foreach ($input in @(
                    @{ path = "tools/archive/playtest06/playtest06_owner_build.ps1"; hash = [string]$ownerBuild.builder_script_sha256; label = "builder_script_sha256" },
                ...
                $requiredOwnerTools = @(
                    "tools/archive/playtest06/playtest06_owner_build.ps1",
                    ...
                    "tools/archive/l02/l02_web_perf_probe.mjs",
                    "tools/serve_web.ps1"
                )
                $actualOwnerTools = @()
                $ownerToolPaths = @{}
