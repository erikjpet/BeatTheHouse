...
const port = Number(args.port ?? 18144);
...
try {
  ...
  page.on("console", (message) => {
    ...
    if (text.startsWith("GAME06_4_MACHINE_GAMES_PLATFORM=")) {
      report = JSON.parse(text.slice("GAME06_4_MACHINE_GAMES_PLATFORM=".length));
  ...
  if (report === null) throw new Error("Timed out waiting for Game 06.4 Web report.");
  ...
  if (!report.ok || report.platform !== "Web" || !report.semantic_sha256) {
    throw new Error(`Game 06.4 Web probe failed: ${JSON.stringify(report.failures ?? [])}`);
  }
  console.log(`GAME06_4_MACHINE_GAMES_WEB PASS output=${outputPath} semantic=${report.semantic_sha256} cpu=${cpu}`);
} finally {
