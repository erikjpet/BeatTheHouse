...
foreach ($map in @(Get-Values $surfaceRoot.maps)) {
    ...
    foreach ($counter in @(Get-Values $map.counters)) {
        if ([string]::IsNullOrWhiteSpace([string]$counter.id) -or [double]$counter.x0 -ge [double]$counter.x1 -or [double]$counter.top_y -lt 0 -or [double]$counter.front_y -lt [double]$counter.top_y) {
