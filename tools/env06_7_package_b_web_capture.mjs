...
const port = Number(args.port ?? 18142);
...
try {
  ...
  page.on("console", (message) => {
    ...
    if (text.startsWith("ENV06_7_PACKAGE_B_PLATFORM=")) report = JSON.parse(text.slice("ENV06_7_PACKAGE_B_PLATFORM=".length));
  ...
  if (report === null) throw new Error(`Timed out waiting for Package B Web report.`);
  ...
  if (consoleErrors.length || pageErrors.length || requestFailures.length) {
    report.ok = false;
    report.failures = [...(report.failures ?? []), `Browser emitted console=${consoleErrors.length} page=${pageErrors.length} request=${requestFailures.length} failures.`];
  }
  fs.writeFileSync(outputPath, JSON.stringify(report, null, 2));
  if (!report.ok || report.platform !== "Web" || !report.semantic_sha256) throw new Error(`Web Package B probe failed: ${JSON.stringify(report.failures ?? [])}`);
  console.log(`ENV06_7_PACKAGE_B_WEB_CAPTURE PASS output=${outputPath} semantic=${report.semantic_sha256} cpu=${cpu}`);
} finally {
