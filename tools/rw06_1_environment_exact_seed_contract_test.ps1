...
function Invoke-Q009SentinelFilesystemCapabilityProbe {
    ...
    try{
        ...
        $chainHead=New-Q009OwnedChildManifestChainHead -RootPath $root -RootIdentity $rootReceipt.identity -AttemptId $AttemptId -Boundary 'capability_operations_complete' -OwnerKind launcher_fixture -OwnerReceipt $capabilityOwner
        ...
        $sentinelState=Get-Q009HeldHandleState $cell sentinel;if(-not(Test-Q009HeldHandleStateShape $sentinelState $sentinelReceipt.identity $false)-or-not[bool]$sentinelState.delete_pending){throw 'Capability sentinel was not exactly delete-pending before final-root handoff'}
...
function Update-Q009RuntimeArtifactManifestChains {
    ...
    $nextCache=New-Q009OwnedChildManifestChainHead -RootPath $CacheRoot -RootIdentity $CacheRootIdentity -AttemptId ([string]$RunContext.attempt_id) -Boundary ('closed_process_cache_'+[string]$Phase.name) -OwnerKind closed_process_phase -OwnerReceipt $cacheReceipt -PreviousHead $CacheChain
...
function New-CrossBindingSelfTestFixture {
    ...
    for($index=0;$index-lt6;$index+=1){
        ...
        $catalog=if($index-lt5){[object[]]@((&$newChoice "node-$($index+1)"),(&$newChoice 'spare-a'),(&$newChoice 'spare-b'))}else{[object[]]@()}
...
function Invoke-ValidateOnlySelfTest {
    ...
    try{
        ...
        Add-SelfTestResult $cases 'runtime-source-custody-lifecycle-provenance-seams' {
            ...
            $successCleanupOrdered=$cacheLifecycle.process_gate-ge0-and$cacheLifecycle.quiescence-gt$cacheLifecycle.process_gate-and$cacheLifecycle.profile_cleanup-gt$cacheLifecycle.quiescence-and$cacheLifecycle.final_pin_proof-gt$cacheLifecycle.profile_cleanup-and$cacheLifecycle.final_pin_proof-gt$cacheLifecycle.import_phase-and$cacheLifecycle.manifest-gt$cacheLifecycle.final_pin_proof-and$cacheLifecycle.ordinary_cleanup-gt$cacheLifecycle.manifest-and$cacheLifecycle.sentinel_disposition-gt$cacheLifecycle.ordinary_cleanup-and$cacheLifecycle.final_root_open-gt$cacheLifecycle.sentinel_disposition-and$cacheLifecycle.sentinel_close-gt$cacheLifecycle.final_root_open-and$cacheLifecycle.root_disposition-gt$cacheLifecycle.sentinel_close-and$cacheLifecycle.root_close-gt$cacheLifecycle.root_disposition-and$cacheLifecycle.final_absence-gt$cacheLifecycle.root_close-and$cacheLifecycle.retain_custody-gt$cacheLifecycle.quiescence
            ...
            foreach($criticalToken in @($rootCreateToken,$sentinelCreateToken,$preHandoffProofToken,$initialRootCloseToken,$postHandoffProofToken,$ordinaryCleanupToken,$sentinelDispositionToken,$finalRootOpenToken,$sentinelCloseToken,$rootDispositionToken,$rootCloseToken,$finalCacheAbsenceToken)){
                ...
                $mutatedSuccess=$mutatedLifecycle.process_gate-ge0-and$mutatedLifecycle.quiescence-gt$mutatedLifecycle.process_gate-and$mutatedLifecycle.final_pin_proof-gt$mutatedLifecycle.quiescence-and$mutatedLifecycle.manifest-gt$mutatedLifecycle.final_pin_proof-and$mutatedLifecycle.ordinary_cleanup-gt$mutatedLifecycle.manifest-and$mutatedLifecycle.sentinel_disposition-gt$mutatedLifecycle.ordinary_cleanup-and$mutatedLifecycle.final_root_open-gt$mutatedLifecycle.sentinel_disposition-and$mutatedLifecycle.sentinel_close-gt$mutatedLifecycle.final_root_open-and$mutatedLifecycle.root_disposition-gt$mutatedLifecycle.sentinel_close-and$mutatedLifecycle.root_close-gt$mutatedLifecycle.root_disposition-and$mutatedLifecycle.final_absence-gt$mutatedLifecycle.root_close
            ...
            foreach($terminalToken in @($prePublicationToken,$terminalSafeToken,$terminalOwnerToken,$terminalOwnerShapeToken,$terminalManifestToken,$summaryConstructionToken,$summaryPublishToken,$postSummaryManifestToken,$releaseShapeToken,$terminalCompletionToken,$terminalCompletionShapeToken,$terminalPublishToken,$finalTreeToken,$finalTreeGateToken,$summaryReceiptToken)){
                ...
                $mutated=$runtimeEntrySource.Remove($position,$terminalToken.Length);$mutatedLifecycle=&$getTerminalLifecyclePositions $mutated
...
'@;[IO.File]::WriteAllText($probe,$probeSource,[Text.UTF8Encoding]::new($false));$ps=Join-Path $PSHOME 'powershell.exe'
        ...
        Add-SelfTestResult $cases 'tip-blob-environment-drift-pure-guards' {
            ...
            $pinPath=Join-Path $tempRoot 'tracked-dependency-pin.txt';$pinOwnership=[ordered]@{created=$false;identity=$null;write_complete=$false;removal_failed=$false;removal_error=''};New-OwnedLeaseFile $pinPath 'held dependency bytes' $pinOwnership
...
try{
    ...
    $cacheArtifactRequest=[ordered]@{role='project_cache';path=[string]$projectCacheRoot;root_identity=$cacheRootIdentity}
    ...
    $importChains=Update-Q009RuntimeArtifactManifestChains $import $runContext $projectCacheRoot $cacheRootIdentity $cacheManifestChain $profileRoot $profileRootIdentity $profileManifestChain $true;$cacheManifestChain=$importChains.cache_chain;$profileManifestChain=$importChains.profile_chain
    ...
    $staticChains=Update-Q009RuntimeArtifactManifestChains $static $runContext $projectCacheRoot $cacheRootIdentity $cacheManifestChain $profileRoot $profileRootIdentity $profileManifestChain $false;$cacheManifestChain=$staticChains.cache_chain;$profileManifestChain=$staticChains.profile_chain
    ...
    if($Mode-eq'Historical22'){
        ...
        foreach($expectation in @($manifest.expectations)){
            ...
            $phaseChains=Update-Q009RuntimeArtifactManifestChains $phase $runContext $projectCacheRoot $cacheRootIdentity $cacheManifestChain $profileRoot $profileRootIdentity $profileManifestChain $true;$cacheManifestChain=$phaseChains.cache_chain;$profileManifestChain=$phaseChains.profile_chain
    ...
    if($phaseResidualFree-and$preCleanupQuiescent){
        ...
        try{
            ...
            $cacheManifestChain=New-Q009OwnedChildManifestChainHead -RootPath $projectCacheRoot -RootIdentity $cacheRootIdentity -AttemptId $attemptId -Boundary 'cache_terminal_after_process_quiescence' -OwnerKind cache_creator -OwnerReceipt $cacheOwner -PreviousHead $cacheManifestChain
