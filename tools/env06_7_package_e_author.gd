...
const CONFIGS := [
	...
	{"id":"delta_queen_engine_trouble","archetype":"delta_queen","arrival":"A dead engine anchors the boat mid-river; the locked gangway, falling pressure gauge, and evacuation benches make every route visible.","verbs":["seal_engine_lane","diagnose_pressure","fetch_impeller_key","choose_engine_response","restart_engine"],"beats":["bulkhead_state","gauge_appearance","bench_move"],"fact":"world_boundary","tags":["travel_locked","power_decay","rising_tension"],"objects":[["engine_bulkhead","Hot engine bulkhead","barrier","center","sealed"],["pressure_gauge","Falling pressure gauge","instrument","left","critical"],["evacuation_benches","Benches","safety","right","empty"],["locked_gangway","Locked mid-river gangway","route_marker","exit_lane","locked"]],"actors":[["engine_mate","Engine mate","actor_engine_mate","left","work"],["deck_guard","Guard","actor_deck_guard","right","guard"]],"outcomes":["travel_resumed","limited_power","evacuation_staged","engine_abandoned"]},
	...
	{"id":"grand_casino_gala_night","archetype":"grand_casino","arrival":"Coat check, charity credentials, and a stalled stage lift replace the normal floor route without touching the duel or invite gate.","verbs":["claim_coat_token","verify_charity_badge","reset_stage_lift","choose_gala_identity"],"beats":["token_move","badge_state","lift_appearance"],"fact":"service_result","tags":["gala_cover","charity_crowd","boss_gate_sacred"],"objects":[["coat_check","Gala coat check","service","left","busy"],["charity_badges","Charity credential desk","credential","center","checking"],["stage_lift","Lift","workstation","right","stalled"]],"actors":[["gala_host","Gala host","actor_gala_host","center","guard"],["stage_hand","Gala stage hand","actor_stage_hand","right","work"]],"outcomes":["cover_identity_accepted","public_floor_reopened","credential_failed","gala_interrupted"]},
	{"id":"grand_casino_convention_crowd","archetype":"grand_casino","arrival":"Badge-wearing delegations reserve two machine banks and collide at one table block, making camouflage and reduced comps publicly legible.","verbs":["read_booking_board","sort_delegation_badges","move_machine_markers","choose_convention_route"],"beats":["board_state","badge_move","delegation_behavior"],"fact":"game_result","tags":["badge_camouflage","comp_reduction","reservation_rotation"],"objects":[["booking_board","Booking Board","ledger","left","conflicted"],["machine_banks","Reserved machine banks","game_station","center","split"],["table_block","Block","barrier","right","crowded"]],"actors":[["convention_coordinator","Coordinator","actor_convention_coordinator","left","work"],["badge_delegations","Badge delegations","actor_badge_delegations","center","watch"]],"outcomes":["camouflage_route","split_schedule","floor_standoff","convention_departed"]},
...
func _entry(c: Dictionary) -> Dictionary:
	...
	scene_ops.append(_scene_spawn(prefix, "arrival", exit_id, "Clear exit", "exit", _spatial_zone(prefix, "safe_exit", "exit_lane"), "marked_lane", 56, 56))
	...
	for index in range(1, c.verbs.size()):
		...
		if is_decision_phase:
			if _decision_has_terminal(decision, "terminal_failure"):
				branches.remove_at(2)
...
func _decision_semantics(prefix: String, phase_id: String, decision: Dictionary, primary_command: String) -> Dictionary:
	...
	for index in range(_array(decision.get("options", [])).size()):
		...
		result.scene.append(_scene_spawn(prefix,phase_id,target_id,"Route %d" % (index + 1),"decision_route",zones[index],"route_%d_ready" % index,68,56))
		...
		if index == 0 and not primary_command.is_empty():
			...
			if not _decision_has_terminal(decision, "terminal_failure"):
				actions.append(_action("fail_%s" % prefix, "Let the pressure win", "ui_right"))
		var interaction := _interaction_add(prefix,phase_id,target_id,"Route %d" % (index + 1),"Use this marked spatial route; the other two stations remain independently focusable.",actions,false)
...
func _decision_has_terminal(decision: Dictionary, terminal_phase: String) -> bool:
	for option_value in _array(decision.get("options", [])):
		if typeof(option_value) == TYPE_ARRAY and str((option_value as Array)[1]) == terminal_phase:
			return true
	return false
