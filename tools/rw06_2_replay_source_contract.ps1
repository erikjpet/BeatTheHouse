...
'@ }
    ...
    Assert-Match $runner '(?s)function Reach-GrandCasino.*?\$grand\s*=\s*@\(\$nodes.*?archetype_id.*?grand_casino.*?\$grand\.Count\s+-gt\s+1.*?travel_enabled.*?Travel-ToNode.*?travel_disabled_reason.*?Not enough bankroll.*?cash=\$cash, route_cost=\$cost.*?Earn the shortfall through visible play before retrying' 'The clean replay must prefer the exact visible Grand route and report its public affordability blocker before spending more on unrelated travel.'
