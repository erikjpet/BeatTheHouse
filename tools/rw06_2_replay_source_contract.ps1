...
'@ }
    ...
    Assert-Match $runner '(?s)function Invoke-BishopGrandDrinkSobrietyDetour.*?event:comped_suite_offer.*?Comped Suite Offer.*?Decline.*?ChoiceId ''decline''.*?afterCompCash\s+-cne\s+\$beforeCompCash.*?afterCompHeat\s+-cne\s+\(\$beforeCompHeat\s+-\s+2\).*?drunkText\s+-cne\s+''100''.*?final sobriety loop' 'The final Bishop sobriety loop must use the exact rendered Comped Suite decline boundary, bind its public economy and settled Drunk projection, and retain one bounded public loop.'
