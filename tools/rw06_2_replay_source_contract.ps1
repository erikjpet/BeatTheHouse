...
'@ }
    ...
    Assert-Match $runner '(?ms)^function Ensure-PunchlineCasinoDiscovered\s*\{.*?Invoke-OverflowRoomActionButton.*?Parking Lot Tip: Follow the tip.*?afterEventVisible.*?Select-GrandFareMachineJamChoice.*?choiceId\s+-cne\s+''wait''.*?Choose-VisibleChoice.*?post-tip machine_jam.*?afterScreen.*?ENVIRONMENT.*?afterEventVisible.*?afterTalkVisible.*?afterTransitionActive.*?Punchline on the public map.*?(?=^function |\z)' 'The overflow Parking Lot Tip may resolve only to an uninterrupted room or the exact rendered machine_jam de-escalation, then must restore a modal-free room before unchanged Punchline discovery verification.'
