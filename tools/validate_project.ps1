...
function Get-Rw061AuditTravelSourceIssues {
    ...
    try{
        # Git may materialize this held source with CRLF on Windows. Normalize
        # only line boundaries before applying the exact indentation/token
        # checks so `$` cannot strand a carriage return outside `[ \t]*`.
...
try {
    ...
    $exactSeedShadowQuarantine = Join-Path ([IO.Path]::GetDirectoryName($exactSeedShadowRoot)) ('.q009-delete-' + $exactSeedValidatorNonce + '-validator-shadow')
