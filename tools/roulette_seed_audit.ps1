param(
    [int]$SeedCount = 120,
    [string]$Output = "res://.tmp/roulette_seed_audit/report.json",
    [switch]$RequireGodot
)
...
if (-not $godot) {
    ...
    Write-Warning "Godot was not found, so roulette seed audit was skipped."
    exit 0
}

& $godot --headless --path $root --script "res://tools/roulette_seed_audit.gd" -- "--seed-count=$SeedCount" "--output=$Output"
exit $LASTEXITCODE
