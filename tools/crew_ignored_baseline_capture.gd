...
const OUTPUT_PATH := "res://.tmp/crew06_5_ignored_run_baseline.json"

func _init() -> void:
	var library := ContentLibraryScript.new()
	library.load()
	if not library.validation_errors.is_empty():
		push_error("Baseline content failed validation: %s" % JSON.stringify(library.validation_errors))
		quit(1)
		return
	var output := {"schema_version": 1, "baseline_commit": "fe27b293", "capture": _capture(library)}
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(OUTPUT_PATH).get_base_dir())
	var file := FileAccess.open(OUTPUT_PATH, FileAccess.WRITE)
	file.store_string(JSON.stringify(output, "  "))
	file.close()
	print("Crew ignored baseline capture complete.")
	quit(0)

func _capture(library: ContentLibrary) -> Dictionary:
	var runs: Array = []
...
func _world_environments(world_map: Dictionary) -> Array:
	...
	for node_value in world_map.get("nodes", []):
		...
		result.append({"id": str(node.get("id", "")), "environment": (node.get("environment", {}) as Dictionary).duplicate(true) if typeof(node.get("environment", {})) == TYPE_DICTIONARY else {}})
	return result
