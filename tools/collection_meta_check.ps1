...
if (-not $godot) {
    ...
    Write-Warning "Godot was not found, so collection meta check could not run."
    exit 0
}
...
& $consoleGodot --headless --path $root --script "res://scripts/tests/collection_meta_check.gd"
exit $LASTEXITCODE
