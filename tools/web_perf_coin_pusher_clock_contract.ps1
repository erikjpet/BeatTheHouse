$script:CoinPusherShippedBodyCount = 160
...
function Test-CoinPusherReinstallClockObservation {
    ...
    return (Test-CoinPusherPropertiesPresent -Value $Observation -Names @("boundary_body_count", "boundary_tray_count", "liveness_before", "liveness_after", "observed_body_count", "observed_tray_count", "conservation")) `
        -and [int]$Observation.boundary_body_count -eq $script:CoinPusherShippedBodyCount `
        ...
        -and (Test-CoinPusherConservationSnapshot -Snapshot $Observation.conservation -ExpectedOrigin $script:CoinPusherShippedBodyCount)
}

function Test-CoinPusherConservationSnapshot {
    param([object]$Snapshot, [int]$ExpectedOrigin = $script:CoinPusherShippedBodyCount)
...
function Test-CoinPusherSurfaceConservationBinding {
    param([int]$BodyCount, [int]$TrayCount, [object]$Snapshot, [int]$ExpectedOrigin = $script:CoinPusherShippedBodyCount)
...
function Test-CoinPusherReducedSampleBoundary {
    ...
    return (Test-CoinPusherPropertiesPresent -Value $Fixture -Names @("body_count")) `
        ...
        -and [int]$Fixture.body_count -eq $script:CoinPusherShippedBodyCount `
        ...
        -and (Test-CoinPusherSurfaceConservationBinding -BodyCount ([int]$Boundary.body_count) -TrayCount ([int]$Boundary.tray_count) -Snapshot $Boundary.conservation -ExpectedOrigin $script:CoinPusherShippedBodyCount) `
        -and (Test-CoinPusherSurfaceConservationBinding -BodyCount ([int]$ScenarioTags.body_count_before) -TrayCount ([int]$ScenarioTags.tray_count_before) -Snapshot $ScenarioTags.conservation_before -ExpectedOrigin $script:CoinPusherShippedBodyCount)
}
...
function Get-CoinPusherPostCollectAccounting {
    ...
    return [ordered]@{
        ...
        valid = [int]$Tags.tray_count_at_accept -eq 0 `
            ...
            -and [int]$Tags.conservation_at_accept.active -eq ($script:CoinPusherShippedBodyCount - 1) `
            ...
            -and (Test-CoinPusherSurfaceConservationBinding -BodyCount ([int]$Tags.body_count_at_accept) -TrayCount ([int]$Tags.tray_count_at_accept) -Snapshot $Tags.conservation_at_accept -ExpectedOrigin $script:CoinPusherShippedBodyCount) `
            -and (Test-CoinPusherSurfaceConservationBinding -BodyCount ([int]$Tags.body_count_after) -TrayCount ([int]$Tags.tray_count_after) -Snapshot $Tags.conservation_after -ExpectedOrigin $script:CoinPusherShippedBodyCount) `
            -and $bodyDelta -ge 0 `
            -and $trayDelta -ge 0 `
