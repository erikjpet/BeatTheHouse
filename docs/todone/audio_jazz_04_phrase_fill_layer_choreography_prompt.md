...
- Implementation: `bcb41789`, integrated by `a94f338d`.
- Verification: the Jazz choreography probe passed, covering two-bar drum exits, one-shot fills, destination landings, layer arcs, deterministic fill priority, interruption, and restore behavior; systems checks also passed.
- Deviation: final listening approval awaits production stems and fills from the engineer.
