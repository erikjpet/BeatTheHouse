...
- Preconditions verified:
  ...
  - `docs/archive/0.4/plans/0.4_release_checklist.md` has the Published section with the
...
- Web package verification:
  ...
  - Served the extracted package with COOP/COEP headers and ran
    `tools/archive/l02/l02_web_perf_probe.mjs` using `bth_perf_plan=l02`, Chrome headless,
...
- Devlog/social:
  ...
  - Added `docs/archive/0.4/plans/0.4_devlog_post.md`; posting remains the owner's action.
- Closeout:
  - Appended Post-Release Verification to
    `docs/archive/0.4/plans/0.4_release_checklist.md`.
  - Marked `docs/archive/0.4/plans/0.4_act1_completion_plan.md` SHIPPED with tag `v0.4.0`.
...
"Published" section of `docs/archive/0.4/plans/0.4_release_checklist.md`) and the owner
...
1. Update `tools/generate_devlog_social.py`'s DEVLOG config for 0.4.0
   ...
   headline features from `docs/archive/0.4/plans/0.4_publish_copy.md` — home &
...
3. Assemble the final devlog text at `docs/archive/0.4/plans/0.4_devlog_post.md` from
   the publish copy: title, body, feature list, and the image filenames to
   attach. Match the voice of the committed style guide.

## 3. Close the loop

1. Update `docs/archive/0.4/plans/0.4_act1_completion_plan.md` status line to SHIPPED
