...
| 1–4 | rw06_1 | `rw06_1_fixed_slot_rooms_prompt.md` | rw06_0 | IN_PROGRESS (pushed integrated core tip `3aa1a771`; semantic authority `0d2837af` is merged and its side branch/worktree are deleted, as is the already-merged overflow-UI side branch; the exact-tip focused run is red with 20 non-weakened assertions and the day-2 capture is 0/3 on an ordinary-room expanded-geometry authority seam, both on a short-lived contract-fix branch; the next casino layer has an exact GU cap ready; Q-004 gave no approval; core authoring remains paused for a reproducible repair; land on `main` by end of 2026-09-24) |
| 1–5 | rw06_2 | `rw06_2_three_endings_prompt.md` | rw06_0 (runs parallel to rw06_1; final pass after rw06_1) | IN_PROGRESS (embedded-Window routing is fixed at product commit `54080a15` / pushed tip `f643859d`, but probe 8 stopped before ready on one GDScript type-inference parse error; the explicit annotation is in progress without Godot and qualifying routes wait for rw06_1) |
...
| Unmerged row branches (`git branch -a`) | 0 | 3 active release-week branches (rw06_1 core, short-lived rw06_1 focused-contract repair, rw06_2) | release orchestrator |
