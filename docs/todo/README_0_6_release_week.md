...
| 1–4 | rw06_1 | `rw06_1_fixed_slot_rooms_prompt.md` | rw06_0 | IN_PROGRESS (pushed core checkpoint `6631a87b`; the compact overflow panel/input plus Back Alley and Motel fixed-slot maps are repaired engine-free, while the semantic authority fix remains on its side branch; Corner Store/Bar/Grand Casino still await a successful 3/3 capture and Q-004 gave no approval; Gas Station Casino authoring is in progress; land on `main` by end of 2026-09-24) |
| 1–5 | rw06_2 | `rw06_2_three_endings_prompt.md` | rw06_0 (runs parallel to rw06_1; final pass after rw06_1) | IN_PROGRESS (the first four Clean replay-policy blockers are fixed through product commit `a2e98c61` / pushed tip `8d3fe210`; probe 5 proved the fully visible click opens the confirmation, then stopped because the public replay catalog omits that dialog's rendered OK/Cancel controls; the narrow harness-only fix is in progress and qualifying routes wait for rw06_1) |
