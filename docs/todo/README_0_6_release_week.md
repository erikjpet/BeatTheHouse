...
| 1–4 | rw06_1 | `rw06_1_fixed_slot_rooms_prompt.md` | rw06_0 | IN_PROGRESS (17-point exact-tip audit checklist at `78dfa8e1`; core CSP candidate coverage plus independent geometry/route proof are in repair; isolated overflow UI tip `d6cc2b85` has one ordinary-action dispatch P1 under additive repair; Godot off pending one clean audited tip) |
...
- 2026-09-23 release orchestrator: rw06_1 remains fail-closed before engine use. Exact-label Corner packing exposed incomplete bounded candidate coverage while the independent core audit required exact reachable composition census, concrete label/body-sweep checks, production modular binding, preferred-slot fallback, transient TalkDock assertions and same-room replay reset. The isolated overflow UI audit accepted every reviewed surface except one P1 that diverted ordinary action IDs into scenario dispatch; an additive fix and regression are in progress. No Godot process has run on these revisions.
