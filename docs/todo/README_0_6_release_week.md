...
| 1–4 | rw06_1 | `rw06_1_fixed_slot_rooms_prompt.md` | rw06_0 | IN_PROGRESS (pushed checkpoint `4ce68e59`; Corner Store/Bar/Grand Casino static slice is 209 snapshots / 1,411 bindings / zero active overflow / zero errors; fresh focused Godot confirms the outstanding overflow authority and compact-action-list failures, and the exact capture is still 0/3; fixes are active on the core and authority-seam branches; Q-004 gave no approval; land on `main` by end of 2026-09-24) |
| 1–5 | rw06_2 | `rw06_2_three_endings_prompt.md` | rw06_0 (runs parallel to rw06_1; final pass after rw06_1) | IN_PROGRESS (the first three Clean replay-policy blockers are fixed through pushed tip `e52cd0f8`; probe 4 confirmed full visibility, then found a harness-only missing wheel-release event at action 7; the narrow fix is in progress and qualifying routes wait for rw06_1) |
