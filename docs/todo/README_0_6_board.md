...
| craps06_3 | `craps06_3_craps_depth_rework_prompt.md` | IN_PROGRESS — PRODUCT FIRST REJECTION / SPLIT REMEDIATION | craps06_1/2; env06_6b for landing; owner-frozen game ritual contract | depth06_1 | Squad CRAPS06_3 + exclusive env-lane integration package; dedicated Integrator | 2026-08-27 | — | Exact rejected product `f5a30b19`/handoff `1b8ae52c` preserved. P1: profiles are unregistered declarations and implementation was improperly assigned to Integrator; gesture vector is unconsumed and tactile travel/bounce/separation/offer-return absent; token-only proof does not execute required lifecycle/platform/environment/hostile behavior. P2: presentation can remain `aiming_throw`; exact registered suite has two row-facing reds. Original squad owns tactile/phase/proof/test remediation. Exclusive env lane owns a separate row payload registering the package, routing `craps_public_facts`, binding all five profiles, and proving distinct responses; never Integrator implementation and never hidden inside frozen env06_6 pieces. |
...
| ID | Prompt | Status | Depends on | Unblocks | Agent | Started | Finished | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| game06_1 | `game06_1_table_machine_ritual_runtime_prompt.md` | IN_PROGRESS — PRODUCT FIRST REJECTION / REMEDIATION ACTIVE | accepted frozen contract `a2760d81`; red main blocks landing only | game06_2..7 | Squad GAME06_1; dedicated Integrator | 2026-08-27 | — | Exact rejected product `44fefe5f` clean/preserved; frozen contract remains accepted. P1: runtime accepts a reduced synthesized command instead of the closed authenticated RitualCommand envelope; result/rejection/fact/receipt/cache shapes and errors are open/ad hoc; handlers can emit another handler's operations/facts; restore accepts open unauthenticated state. Original squad returned for the ordinary first-remediation round with exact closed-shape, authority, hostile cross-handler, and hostile restore/migration proofs required. |
...
| world06_2 | `world06_2_streets_sequences_prompt.md` | IN_PROGRESS — INDEPENDENT MODEL PACKAGE | world06_1 for receipt bridge/review/landing; Director-authorized independent model work may proceed | world06_6 | Codex/lane:world06_2 | 2026-08-27 | — | Director-authorized lane rotation from parked world06_1: implement delivery-owned physical cargo/location/multi-stop/hold/pursuit model and focused pure-model contracts only. Receipt-aware RunState bridge, adapter integration, review and landing remain dependency-held on the world06_1 authority ruling. |
