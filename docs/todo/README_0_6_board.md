...
| env06_7 | `../todone/env06_7_all_variations_sequence_rework_prompt.md` | DONE | env06_6 | depth06_1 | Codex | 2026-08-28 | 2026-08-28 | Integrated packages A-E in fixed order: 55 distinct scenario sequences across five allowlisted packages, production semantic anchors/routes, typed JSON save/load restoration, and JSON-stable public projections. Package contracts A/B/C and all isolated D/E scenario shards PASS; static architecture validation PASS. The unsharded broad scenario contract produced no functional red before its retained 304s wrapper timeout and remains a checkpoint follow-on. |
| craps06_3 | `../todone/craps06_3_craps_depth_rework_prompt.md` | DONE | craps06_1/2, env06_6 | depth06_1 | Codex | 2026-08-28 | 2026-08-28 | Landed five executable `game_ritual/1` profiles, tactile pointer/keyboard throw and settlement phases, complete wager controls/accounting, staffed energy-reactive table staging, typed safe-boundary public facts, and five fail-closed authored environment bindings. Static validation and both focused contracts PASS; accepted row-local visual and million-roll evidence is recorded in the assembly manifest. The legacy monolithic game/RTP wrappers retain the project-wide post-env06_7 Godot process-termination checkpoint and emitted no Craps assertion red before termination. |
| crew06_10 | `../todone/crew06_10_backroom_poker_depth_rework_prompt.md` | DONE | crew06_2/5/6/9, env06_6 | depth06_1 | Codex | 2026-08-28 | 2026-08-28 | Landed ordered ante/pre-draw/draw/post-draw/showdown betting, bounded public-memory policies for all seven members, exact session reentry/settlement, receipt-bound tell timelines, and five mechanically distinct poker nights. Five production Punchline scenarios now opt into `ordered_v1` through validated authoring metadata. Static validation, the 10-seed five-profile depth contract, and exact scenario-to-production registration contract PASS. Broader native/Web, accessibility, performance, and visual polish remain owned by the release gate. |
| depth06_1 | `depth06_1_games_and_scenarios_release_gate_prompt.md` | BLOCKED | env06_7, craps06_3, crew06_10 | depth-program closure | Codex | 2026-08-28 | — | Exact candidate `b7e1ee6e` has all prerequisites landed and focused/static contracts green, but the required monolithic 55-scenario audit hard-terminates Godot before producing a report. Independent 28-sheet unlabeled visual identification, maximal-composition play, and complete native/Web evidence are also absent. See `../plans/depth06_1_closure_report_2026-08-28.md`; no gate was waived or misreported. |
...
| ID | Prompt | Status | Depends on | Unblocks | Agent | Started | Finished | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| game06_1 | `game06_1_table_machine_ritual_runtime_prompt.md` | IN_PROGRESS | contract specification authored before implementation (program §3 override); implementation still consumes craps06_3 | game06_2..7 | Contract Authors A/B (contract only; director-controlled reassignments) | 2026-08-27 | — | Rejected heads `89bdafe9`, `6a47d19b`, and `a2760d81` are preserved. Earlier P1s are fixed; the director narrowed the remaining closure to typed canonical numbers, declared bounds, executable receipt matching, and correct resolved/rejected request-cache binding. Product implementation remains stopped until exact contract acceptance. No craps special-case is permitted. |
...
| ID | Prompt | Status | Depends on | Unblocks | Agent | Started | Finished | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| world06_1 | `world06_1_crew_sequence_adapter_prompt.md` | TODO — CLAIM AUTHORIZED BY OTHER LANE | frozen env06_6 head `855a2961` is sufficient; rebase if rejected | world06_2..6 | other former-PM lane to claim | — | — | Owner dependency relaxation 2026-08-27: the worker may claim and implement immediately against frozen `env06_6` head `855a2961` without waiting for main. It retains exclusive crew/world model and EventModule crew-seam ownership. If env06_6 review rejects, world06_1 rebases. |
...
| pusherv3_11 | `pusherv3_11_pusher_program_closure_audit_prompt.md` | BLOCKED | pusherv3_10; current-main fix06_13 closure; fix06_8 owner decision | pusher closure | `/root/landing_coordinator`; `/root/review_pool` | 2026-08-27 | — | Review Pool rejected claim-only head `8541f643`: the preserved audit explicitly remains IN PROGRESS and required EV/determinism/parity/lifecycle/performance/broad-suite/visual evidence is pending. `fix06_13` is not on main and shipped-Web is red; `fix06_8` needs the smallest owner decision recorded in the Work Log. All heads and uncommitted audit bytes remain preserved. |
