...
| fix06_9 | `fix06_9_coin_pusher_web_performance_evidence_prompt.md` | REOPENED / PARKED | pusherv3_11 finding | fix06_13 | Codex | 2026-08-26 | — | Evidence remediation head `1b1c509c`: COLLECT and reduced motion now reinstall exact 300-origin fixtures; console policy explicitly allows only the known autoplay warning and rejects hostile/unclassified warnings. First actual shipped-Web run at `54e6398a` remains preserved and materially red, so this row parks on product defect `fix06_13`; no cap was changed and no result was hidden. |
| fix06_13 | `fix06_13_coin_pusher_shipped_web_performance_prompt.md` | IN PROGRESS | accepted fix06_9 evidence remediation | fix06_9, pusherv3_11 closure | Codex agent `/root/fix06_13_web_perf` | 2026-08-27 | — | Claimed from exact main `112cb02d` in `D:\bth-f13` on `codex/land06-fix06_13`. Retained first red remains immutable; one accepted-harness reproduction and attribution profile precede product edits. Locked caps and all gameplay/solver/economy/geometry/tuning/evidence boundaries remain unchanged. |
| fix06_14 | `fix06_14_coin_pusher_collect_reinstall_clock_evidence_prompt.md` | ROUTED / TODO | fix06_13 shipped-Web evidence | fix06_9 evidence closure | Unclaimed | 2026-08-27 | — | Iteration-3 evidence proved the locked COLLECT/reinstall assertions conflict with legitimate production-clock motion: the immutable baseline/reproduction and optimized heads all move the exact 300-body fixture to 296 during normal idle; COLLECT begins from the required conserved 299+tray1 state, empties that result, then new physical exits create tray results during the locked 60-frame window; re-entry similarly advances before identity capture. Preserve gameplay, fixture, caps and locked harness; independently determine the smallest production-clock/evidence synchronization correction. |
