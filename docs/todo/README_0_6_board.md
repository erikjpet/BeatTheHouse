...
| env06_7 | `env06_7_all_variations_sequence_rework_prompt.md` | IN_PROGRESS — OWNER PAUSED / PACKAGES A/B/C/D FIRST REJECTION | accepted env06_6b plus ordered real catalog authority; Smoke-green main | depth06_1 | Squads A-E + ordered assembly; all branches preserved | 2026-08-27 | — | All work paused on red main. D remediation parked clean UNREVIEWED `06d4c818` after identity topology/proof expansion; its combined run has no verdict. A `599e7f56`, B `b1c9c567`, C `ca41e6a2`, D original `ce6fac13` each have one rejection; A was not started. B remains dependency/assembly-held. Binding A/B/C order unchanged. |
| craps06_3 | `craps06_3_craps_depth_rework_prompt.md` | IN_PROGRESS — OWNER PAUSED / WIP PARKED | craps06_1/2; env06_6b; owner-frozen game ritual contract; Smoke-green main | depth06_1 | Squad CRAPS06_3 — `codex/craps06_3-impl` | 2026-08-27 | — | Parked clean on attached named branch at UNREVIEWED `5efea7ce`; prior WIP `4ae3d339` and `1d5dd3d5` preserved. No tests continued after the owner pause. |
...
| ID | Prompt | Status | Depends on | Unblocks | Agent | Started | Finished | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| game06_1 | `game06_1_table_machine_ritual_runtime_prompt.md` | IN_PROGRESS — OWNER PAUSED / WIP PARKED | accepted frozen contract `a2760d81`; Smoke-green main | game06_2..7 | Squad GAME06_1 — `codex/game06_1-impl` | 2026-08-27 | — | Contract remains accepted exact `a2760d81`. Product squad parked clean on attached named branch at immutable UNREVIEWED `804a033c`; commit contains only the initial 537-line closed ritual schema validator. No tests or further implementation continued after the red-main pause. Contract files remain frozen. |
...
| ID | Prompt | Status | Depends on | Unblocks | Agent | Started | Finished | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| world06_1 | `world06_1_crew_sequence_adapter_prompt.md` | BLOCKED — SECOND REJECTION / OWNER A-B DECISION | owner explicitly permits lane continuation; authority boundary decision required; landing still depends on env06_6b/green main | world06_2..6 | parallel former-PM lane; dedicated Integrator | 2026-08-27 | — | Exact `ef55807d` preserved. Choose A: closed DeliveryRunModel checkpoint atomic with `world_applied`/consequences, explicit legacy rule, never synthesize from boolean. Or B: closed `registration.owner_apply_checkpoints` in RunState, atomic with pending receipt plus delivery identity/resolution/receipt fingerprints, no model schema change, ambiguous legacy state fails closed. Both reject transplant/conflict and preserve economy/RNG/tuning. No third ordinary round before owner choice. |
