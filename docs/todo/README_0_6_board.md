...
| polish06_0 | `../todone/polish06_0_post_playtest_program_prompt.md` | DONE | — | the parked second half | PM:Codex/sub:polish | 2026-08-25 | 2026-08-26 | Independent review accepted exact `be14c1ce`; non-ff merge `dba53fb5` parked the post-playtest program, triage/tuning/cleanup prompts and release templates without starting polish or release work. |
...
- **pusherv3_10 — stacked support sentence and Plinko cup lifecycle: ANSWERED
  ...
  placement and bounded reproduction must protect flow and ROI. Production
  citations: `coin_pusher_solver.gd` tracks unilateral `support_ids`, explicit
  `supported_bad`/`bed_level_good` impacts and `cup_consumed_count`;
  `coin_pusher.gd` enqueues multiplier children from `source_nozzle_id`.
