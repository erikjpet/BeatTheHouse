...
| fix06_5 | — (binding timing-audit contract required before claim) | TODO | — | reliable Contract timing guard | PM-routed | — | — | Exact main `7c748f5b` completed all 16 Contract checks with zero functional failures/stderr on `native_v3`, but measured 259.847s against 230.391s. Run at least five times on an otherwise idle host, report every result and record date/host/method before considering any rebaseline; never raise the cap to clear one red. Does not block other rows. |
| fix06_6 | `../todone/fix06_6_delivery_full_state_golden_closure_prompt.md` | IN_PROGRESS | `fix06_4` landed | — | PM:Codex/sub:preflight_inventory | 2026-08-26 | — | Historical product disposition verified: exact-head proof classified the two-hash delivery failure as authored Coin Pusher persisted-state fixture drift; owner-authorized refresh `f9eaebbf` was independently accepted within `fix06_4`, landed at `9a2022ae`, and remained green through exact main `7c748f5b`. The separate-route documentation closeout remains open pending independent acceptance, main landing, post-land documentation check and final record. No code, test, golden, product, capture or runtime gate action belongs to this row. |
| fix06_8 | `fix06_8_coin_pusher_upper_row_join_evidence_prompt.md` | PARKED | `pusherv3_10` landed | Plan 9.4 closure evidence | PM:Codex/sub:fix08_capture_rca | 2026-08-26 | — | Static head `be15d864` is accepted, but actual-GL qualification proved no fixed production drop lands beside qualified local stock (nearest 8.04k/12.43k/17.36k vs 4.70k contact width). Owner must authorize a deterministic production control/nozzle trace that physically reaches adjacent stock or a product/design change. Seed/control search and adjacency weakening remain forbidden; evidence is preserved under `.tmp/fix06_8_exact_production_diag/`. |
