...
| fix06_13 | `fix06_13_coin_pusher_shipped_web_performance_prompt.md` | PARKED | accepted fix06_9 evidence remediation | fix06_9, pusherv3_11 closure | `/root/fix06_13_web_perf` | 2026-08-27 | — | Off-main implementation `7ec148e4d9a6096627fa26e1afee508e5b1c0b25` and exact review/docs head `718af3da7176abfaec9f9dbc10884454298a9872` were independently accepted, but nothing from that branch is accepted on or transplanted to `main`. The sole accepted-head locked run at `c914546f` remained red and exposed clock/fixture evidence contradictions owned by `fix06_14`, now row-owned green pending landing. Unpark integration only after `fix06_14` lands; `fix06_16` must also land before consuming the new locked shipped-Web run. Then integrate semantically onto current `main`, run current-main gates, obtain fresh independent review, and consume exactly one new locked run. Every red and hash remains retained on `codex/land06-fix06_13` and under its ignored evidence paths. |
| fix06_14 | `fix06_14_coin_pusher_collect_reinstall_clock_evidence_prompt.md` | IN PROGRESS / FINAL DOC REVIEW PENDING | fix06_13 shipped-Web evidence | fix06_9 evidence closure | `/root/fix06_14_clock` | 2026-08-27 | — | Exact current-main candidate `dba07c1a` consumed one fresh Chrome 151 CPU4 qualification with all eight scenarios. Every row-owned fixture, conservation, COLLECT terminal, reduced-motion schema/draw/solver and scheduler assertion passed: idle scheduler elapsed 15,971ms with 16 redraws; reduced scheduler/redraw 0/0 with 120 real draws. Exact-head validation/import/load/focused Coin Pusher also passed in 48.214/17.835/24.644/164.306s with the canonical native DLL and zero failures/stderr issues. Exactly 20 retained failures are readiness/frame/draw/resolve performance reds owned by `fix06_13`; no cap changed and no rerun occurred. The wrapper wrote complete evidence before outer orchestration hung on orphan Python PID 39916 / port 18117; the PM stopped that exact process, verified cleanup, and routed lifecycle defect `fix06_16`. Final docs review and main landing remain required before DONE. |
| fix06_16 | `fix06_16_web_perf_server_orphan_cleanup_prompt.md` | TODO | fix06_14 exact-head qualification | fix06_13 unparked locked run | — | 2026-08-27 | — | Routed lifecycle defect: after `web_perf_smoke.ps1` wrote the complete report and summary, `serve_web.ps1` left orphan Python PID 39916 listening on port 18117 and outer orchestration remained alive until 904s. Preserve the consumed evidence; make wrapper cleanup deterministic and fail closed before the next locked `fix06_13` run. |
