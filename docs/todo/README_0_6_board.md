...
| ID | Prompt | Status | Depends on | Unblocks | Agent | Started | Finished | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| env06_6 | `env06_6_dynamic_scenario_runtime_prompt.md` | IN_PROGRESS — PIECE 1 ACCEPTED / LANDING PAUSED ON RED MAIN | env06_1/2/3/5 (DONE); Smoke-green main | env06_6b | PM:Codex/sub:runtime-owner; dedicated Integrator | 2026-08-25 | — | Piece 1 is accepted exact head `855a2961` and has landing priority ahead of every env06_7 package, but the owner ordered the entire plan paused if current main is red; current main `3f5e9907` is Smoke-red. Do not merge until main is Smoke-green. Nothing on main yet consumes this known-incomplete runtime. |
...
| env06_7 | `env06_7_all_variations_sequence_rework_prompt.md` | IN_PROGRESS — D HANDED OFF / A REMEDIATION ACTIVE | accepted env06_6b plus ordered real catalog authority for landing | depth06_1 | Squads A-E + ordered assembly; all branches preserved | 2026-08-27 | — | D first-rejection remediation product `8c2fc885`, handoff `20a445f4`: author/66-pair uniqueness PASS, executable matrix PASS 122.1s, validation PASS 58.993s; queued for Integrator, dependency-held. A remediation active from `599e7f56`; C queues next; B waits on real ordered assembly authority. A/B/C/D each have one rejection. Binding A→B→C order unchanged. |
| craps06_3 | `craps06_3_craps_depth_rework_prompt.md` | IN_PROGRESS — WIP PARKED FOR CREW ROTATION | craps06_1/2; env06_6b for landing; owner-frozen game ritual contract | depth06_1 | Squad CRAPS06_3 — `codex/craps06_3-impl` | 2026-08-27 | — | Clean UNREVIEWED `f5a30b19`: five-profile ritual package, depth contract, bet matrix, assembly manifest, warning/fact integration. Focused ritual proof and module parse PASS; broad validation remains incomplete after JSON-root correction. Slot rotated to crew06_10. |
| crew06_10 | `crew06_10_backroom_poker_depth_rework_prompt.md` | IN_PROGRESS — SQUAD RESUMED | crew06_2/5/6/9; env06_6b for landing; owner-frozen game ritual contract | depth06_1 | Squad CREW06_10 — `/root/env06_7_pkg_c`, `codex/crew06_10-impl` | 2026-08-27 | — | Resumed from clean immutable UNREVIEWED `a0090874`. Squad is row-local; crew/world models and EventModule remain exclusively with the parallel lane. Red main blocks landing only. |
...
| ID | Prompt | Status | Depends on | Unblocks | Agent | Started | Finished | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| game06_1 | `game06_1_table_machine_ritual_runtime_prompt.md` | IN_PROGRESS — WIP PARKED / CAPACITY QUEUED | accepted frozen contract `a2760d81`; red main blocks landing only | game06_2..7 | Squad GAME06_1 — `codex/game06_1-impl` | 2026-08-27 | — | Contract accepted exact `a2760d81`; product WIP parked clean at `804a033c` with the initial closed schema validator. Resumes when the short main-health slot releases. Contract files remain frozen. |
...
| ID | Prompt | Status | Depends on | Unblocks | Agent | Started | Finished | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| world06_1 | `world06_1_crew_sequence_adapter_prompt.md` | IN_PROGRESS — OWNER RULING A / REMEDIATION AUTHORIZED | sealed DeliveryRunModel checkpoint; env06_6b/green main only for landing | world06_2..6 | parallel former-PM lane; dedicated Integrator | 2026-08-27 | — | Exact rejected `ef55807d` preserved. Owner requires the causal checkpoint inside DeliveryRunModel, atomic with `world_applied`/consequences, with explicit fail-closed legacy migration and no boolean synthesis. Adapter authority is rejected. If the sealed model cannot represent a required state, report that exact state; never route around it. |
...
| fix06_8 | `fix06_8_coin_pusher_upper_row_join_evidence_prompt.md` | IN_PROGRESS — OWNER OPTION A / CAPACITY QUEUED | unchanged production pusher physics and geometry | pusherv3_11 | preserved `codex/land06-fix06_8` at `c821d5a6`; next released squad | 2026-08-27 | — | Deterministic production control/nozzle trace only. Must prove qualified adjacent platform-rooted stock moves; failed proof becomes a separately evidenced product-physics defect and owner escalation, not permission to change physics. |
| fix06_21 | `fix06_21_main_smoke_hash_and_post_land_gate_prompt.md` | IN_PROGRESS | exact red main `3f5e9907`; owner authorization | Smoke-green main; env06_6 piece-1 landing | Main-health correction squad; dedicated Integrator | 2026-08-27 | — | Exact hash commit `e3f6e94a` changes only the two expectations and records full old/new hashes, sole duplicated Quarter Falls copy leaf delta, source `737a8fe9`, and landing `040f9fe2`. Independent proof and fail-closed post-land process commit remain active; no product/runtime/budget change. |
