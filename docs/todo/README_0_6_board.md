...
| ID | Prompt | Status | Depends on | Unblocks | Agent | Started | Finished | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| feat06_1 | `feat06_1_scratch_scalper_ticket_trade_prompt.md` | IN_PROGRESS | sections 1.1-1.5 none; section 1.6 needs fix06_31 merged | scratch-ticket depth; heat sink | `/root` | 2026-09-12 | — | Claimed from clean local `main` `00c52ce6` after confirming `fix06_31` is merged to `origin/main`; all sections are in scope. Owner-directed new mechanic 2026-09-11, outside the original 0.6 program. The scratch scalper already exists (30% encounter, 50/50 informed dialogue, empty-machine guarantee, restock interception). This row adds: a conversation option to hand him an unscratched ticket for -8 heat and a 33% low-tier item, bounded to one gift per encounter so it cannot become a heat-laundering exploit; and a chance for scalpers to arrive at a restock boundary rather than only on player arrival. Reward must be a within-run item, not a cross-run collection drop (`content06_1` ruling). Section 1.6 adds owner-directed arrival/departure movement for environment people: someone entering walks from a doorway to their place and someone leaving walks out, reusing the existing actor `route_stage` machinery and `fix06_31`'s authored `doorways`/`floor.bands`. Presentation only, non-interactive in transit, settled geometry untouched, reduced-motion parity, mid-visit changes only. |
| fix06_32 | `fix06_32_unreviewed_game_rework_verification_prompt.md` | TODO | — (parallel-safe with fix06_31) | trustworthy wager math; regression-protected game work | — | — | — | PM audit 2026-09-11 of commits `f870dd21`..`c570f2ce`: ~6,000 lines of craps/Hold'em/coin-pusher/slot/blackjack change landed with no row, no work-log entry and no independent review. Confirmed gaps: 40 player-reachable craps wagers but only 9 with documented house edge and only 9 RTP-measured (every proposition, hardway, buy and lay bet unmeasured); 8 new verification tools wired into no suite; `3f31bf10` taught `scenario_sequence_schema.gd` to waive the 3-outcome and 3-aftermath depth gates via `owner_exceptions` with zero consumers. Verification row, not a redo. |
