...
| fix06_13 | `fix06_13_coin_pusher_shipped_web_performance_prompt.md` | PARKED | accepted fix06_9 evidence remediation | fix06_9, pusherv3_11 closure | `/root/fix06_13_web_perf` | 2026-08-27 | — | Off-main implementation `7ec148e4d9a6096627fa26e1afee508e5b1c0b25` and exact review/docs head `718af3da7176abfaec9f9dbc10884454298a9872` were independently accepted, but nothing from that branch is accepted on or transplanted to `main`. Its retained locked run at `c914546f` remained red. The clock/evidence dependency `fix06_14` is now DONE on main at `616b5a76`; `fix06_13` remains parked behind TODO lifecycle row `fix06_16`, which must land before the new locked shipped-Web run. Then integrate semantically onto current `main`, run current-main gates, obtain fresh independent review, and consume exactly one new locked run. Every red and hash remains retained on `codex/land06-fix06_13` and under its ignored evidence paths. |
| fix06_14 | `fix06_14_coin_pusher_collect_reinstall_clock_evidence_prompt.md` | DONE | fix06_13 shipped-Web evidence | fix06_9 evidence closure | `/root/fix06_14_clock` | 2026-08-27 | 2026-08-27 | Landed by merge `616b5a76` with parents current main `7cd6a5cb` and accepted integration/docs `5917baf2`; owner WIP remained untouched. The first disposable-checkout post-land gate retained an environmental disk-full red after validation passed 51.033s: import failed in 43.037s with exit 127 and 74 stderr issues while D: had 270,336 bytes free. Evidence was moved before removing only the disposable worktree, restoring 1,054,285,824 bytes free. On the identical exact main and native DLL, the single corrected retry passed validation/import/load/focused Coin Pusher in 47.832/17.645/23.346/164.588s with zero failures. `fix06_14` is landed and post-land green; the separately routed server lifecycle defect remains TODO `fix06_16`, and `fix06_13` remains parked behind it. |
| fix06_16 | `fix06_16_web_perf_server_orphan_cleanup_prompt.md` | IN_PROGRESS | fix06_14 exact-head qualification | fix06_13 unparked locked run | `/root/fix06_16_impl` | 2026-08-27 | — | Exact-base bounded reproduction confirmed stopping wrapper PID 2380 left direct Python PID 12964 listening on port 64488; only those exact PIDs were cleaned. Reviews rejected `239ead2a` (native quoting), `e92fab10` (expected-exit classification), and `672cdd25` (one-way marker race plus wrapper PID-reuse window). Current remediation requires an exact live-child nonce acknowledgement before termination and revalidates wrapper start identity immediately before fallback. Deterministic race/PID-reuse hostile tests, parser, Python compilation and static validation pass; no browser/Web timing/export ran. Exact-head re-review required. |
