...
| fix06_18 | `fix06_18_scenario_caller_transaction_rollback_prompt.md` | TODO | env06_6 deferred runner `c373bb52` | env06_6 content acceptance | unclaimed | 2026-08-27 | — | Routed from the single clean accepted-head filtered-content run. Exactly eleven public scenario entry/travel callers fail exact enclosing-state rollback and terminal-error ordering; seven UI paths also attempt focus while detached from the SceneTree. Evidence and hashes are locked in the row prompt. No caller runtime change is part of env06_6. |
