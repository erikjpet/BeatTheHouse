...
| fix06_13 | `fix06_13_coin_pusher_shipped_web_performance_prompt.md` | INLINE LEAK SUCCESSOR IN REVIEW | accepted fix06_9 evidence remediation; fix06_14/16 DONE | fix06_9, pusherv3_11 closure | Squad FIX06_13; Second Integrator | 2026-08-27 | — | Frozen `62d45c2e` functional red is preserved. Exact successor `fc05b8ee`/tree `f6fd4bfe` changes only the native step kernel and focused proof: candidate-added process-lifetime cache retained per-call `RngStream`; both cached paths now clear only transient config while preserving numeric solver state, with a weakref ownership negative. No local rebuild/gate/Web run occurred. Independent review is active; if accepted, Integrator rebuild/hash plus one functional rerun precedes the still-unconsumed single locked Web result. |
...
| fix06_24 | `fix06_24_coin_pusher_adjacent_coin_movement_defect_prompt.md` | BLOCKED — PRODUCT-PHYSICS DEFECT / OWNER DECISION | exact fix06_8 evidence `1f0595af` | pusherv3_11 | evidence squad; owner authorization required | 2026-08-27 | — | Docs-only option-neutral packet exact `d2a860da` records the choice: authorize a bounded product-physics remediation that makes a fixed production control/nozzle trace land with platform-rooted first support strictly adjacent to qualified pre-existing platform-rooted coin(s) and move those exact neighbors over a phase-matched full stroke; or explicitly revise/cut Plan 9.4. No rerun/search or physics/geometry/nozzle/transport/contact/neighboring-stock change is authorized before the decision. |
