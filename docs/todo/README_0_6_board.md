...
| fix06_15 | `fix06_15_foundation_split_parser_routing_prompt.md` | TODO | inherited `c507475d` Foundation split | env06_6 shared full-gate acceptance | — | — | — | Routed release-suite infrastructure defect: direct `check_core_content.gd` load retained 668 parse errors, including 592 unresolved-call occurrences across 102 unique helpers/checks assigned to later split scripts but unreachable from the root caller. Restore coherent ownership/routing without weakening, deleting, filtering, skipping or duplicating a check. |
