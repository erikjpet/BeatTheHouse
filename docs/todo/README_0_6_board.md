...
| fix06_16 | `fix06_16_web_perf_server_orphan_cleanup_prompt.md` | IN_PROGRESS | fix06_14 exact-head qualification | fix06_13 unparked locked run | `/root/fix06_16_impl` | 2026-08-27 | — | Exact-base bounded reproduction confirmed stopping wrapper PID 2380 left direct Python PID 12964 listening on port 64488; only those exact PIDs were cleaned. Independent review rejected `239ead2a` P2 for unquoted Windows native arguments. Remediation now quotes both launch boundaries and passes a hostile real-server test with spaced script/root/ownership/stdout/stderr paths, exact headers/content/cleanup and unrelated-process survival. Expanded suite, parsing and static validation pass; no browser/Web timing/export ran. Exact-head re-review required. |
