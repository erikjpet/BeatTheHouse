...
| fix06_16 | `fix06_16_web_perf_server_orphan_cleanup_prompt.md` | IN_PROGRESS | fix06_14 exact-head qualification | fix06_13 unparked locked run | `/root/fix06_16_impl` | 2026-08-27 | — | Exact-base bounded reproduction confirmed stopping wrapper PID 2380 left direct Python PID 12964 listening on port 64488; only those exact PIDs were cleaned. Reviews rejected `239ead2a` for unquoted Windows native arguments and `e92fab10` because intentional cleanup could emit a blank-code lifecycle error. Remediation now quotes both boundaries and crosses a nonce-bound shutdown marker before exact termination; expected paths have no lifecycle stderr and unrequested child exit remains an explicit failure. Hostile real-server suite passes; no browser/Web timing/export ran. Exact-head re-review required. |
