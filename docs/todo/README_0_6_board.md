...
| ID | Prompt | Status | Depends on | Unblocks | Agent | Started | Finished | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| fix06_28 | `../todone/fix06_28_playtest_blocking_interaction_and_surface_pass_prompt.md` | DONE | owner playtest | playable 0.6 owner build | `/root` | 2026-09-07 | 2026-09-07 | Production-host, exact-id, real-mouse working-order proof passed 3 seeds with 130 rendered selections and 74 actions, repeated travel/revisit and Save/Continue; all six surface families, focused Crew, Punchline, slot and machine checks are green. Every accepted scenario action now has visible acknowledgement/state consequence, hidden state stays sealed, and split runners fail closed. The former false green bypassed the room/input host, accepted invisible generic deltas, and ignored 65 Crew failures. Local Windows build follows exact-head validation. Absorbs and completes `env06_9`; report: `../plans/fix06_28_playtest_blocking_interaction_and_surface_pass_report.md`. |
| fix06_29 | `fix06_29_punchline_natural_l3_route_prompt.md` | TODO | owner playtest / naturally earned made Crew standing or Rook escort save | Punchline progression refinement | — | — | — | Exercise the exact L3 back-room route through naturally earned progression with production host, exact ids, real input, persistence and visible-only evidence. Not a local-build blocker. |
| fix06_30 | `fix06_30_foundation_contract_runner_lifecycle_prompt.md` | TODO | owner playtest | broad Foundation harness integrity | — | — | — | Repair the canonical composed `contracts` runner's nil lifecycle-helper errors and 300-second no-verdict. Focused required product/slot gates are green; this follow-up may not claim PASS until the broad command returns a genuine zero-failure verdict. |
