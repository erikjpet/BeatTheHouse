...
| fix06_10 | `fix06_10_coin_pusher_ev_evidence_runner_oom_prompt.md` | IN_PROGRESS | `pusherv3_10` landed; retained `pusherv3_11` EV failure | trustworthy `pusherv3_11` economics evidence | PM:Codex/sub:ev_rca_remediation | 2026-08-26 | — | Six parallel persistent-machine shards exhausted host memory and the fail-open wrapper advanced after missing JSON. Harness-only head `9a6c6c97` was rejected on stale-output, cleanup, immediate-parse and successful-memory-evidence gaps; `/root/fix10_review` accepted remediated exact head `861d2d40` with no P0–P3 findings. Exact serial 200k accepted inserts per machine, main landing and post-land record remain required; no sample, economy, RNG, payout, physics or cap changed. |
