...
| ID | Prompt | Status | Depends on | Unblocks | Agent | Started | Finished | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| game06_1 | `game06_1_table_machine_ritual_runtime_prompt.md` | IN_PROGRESS — PRODUCT SQUAD RESUMED | accepted frozen contract `a2760d81`; red main blocks landing only | game06_2..7 | Squad GAME06_1 — `/root/env06_7_pkg_c`, `codex/game06_1-impl` | 2026-08-27 | — | Product squad resumed from clean UNREVIEWED `804a033c`. It owns the Family 1 shared ritual runtime/catalog/master probes; contract files remain frozen. No env/crew-world/shared-assembly edit permitted. |
...
| fix06_8 | `fix06_8_coin_pusher_upper_row_join_evidence_prompt.md` | IN_PROGRESS — OWNER OPTION A / SLOT ROTATING TO FIX06_22 | unchanged production pusher physics and geometry | pusherv3_11 | preserved `codex/land06-fix06_8` at `c821d5a6`; standing evidence squad | 2026-08-27 | — | Deterministic production control/nozzle trace only. Squad checkpoints clean and temporarily rotates to remediate near-ready fix06_22 first rejection. No physics/geometry/tuning change; failed adjacent movement proof becomes a separately evidenced owner escalation. |
...
| fix06_22 | `fix06_22_fail_closed_post_land_verification_prompt.md` | IN_PROGRESS — FIRST REJECTION REMEDIATION | exact current main `a62f4884`; performance health before landing | reliable DONE eligibility | Post-land process squad; dedicated Integrator | 2026-08-27 | — | Integrator rejected exact `e75f6c40` P1 before gates: `-NoImport` can silently narrow PostLand, and supplied plugin hash is not bound to the DLL target actually loaded from `.gdextension`. Remediation must force/record import narrowing red and bind canonical descriptor target/hash with hostile duplicate-path proof. Exact head preserved; no product/runtime/budget/assertion finding. |
