...
| env06_7 | `env06_7_all_variations_sequence_rework_prompt.md` | IN_PROGRESS — A/D HANDED OFF / C REMEDIATION ACTIVE | accepted env06_6b plus ordered real catalog authority for landing | depth06_1 | Squads A-E + ordered assembly; all branches preserved | 2026-08-27 | — | A product `db8abbd1`/handoff `6f5b11b5`: executable 12-id/66-route/11-fact matrix PASS 20.3s, validation 58.670s. D product `8c2fc885`/handoff `20a445f4`: matrix PASS 122.1s, validation 58.993s. Both queued for Integrator and dependency-held. C remediation active from `ca41e6a2`; B waits on real ordered assembly authority. A/B/C/D each have one rejection; A→B→C order unchanged. |
...
| fix06_8 | `fix06_8_coin_pusher_upper_row_join_evidence_prompt.md` | IN_PROGRESS — OWNER OPTION A / SQUAD ACTIVE | unchanged production pusher physics and geometry | pusherv3_11 | preserved `codex/land06-fix06_8` at `c821d5a6`; standing evidence squad | 2026-08-27 | — | Deterministic production control/nozzle trace only. Squad resumed after fix06_22 remediation handoff. No physics/geometry/tuning change; failed qualified adjacent movement proof becomes a separately evidenced owner escalation, never an implicit correction. |
...
| fix06_22 | `fix06_22_fail_closed_post_land_verification_prompt.md` | INTEGRATOR SECOND REVIEW — REMEDIATION READY | exact current main `a62f4884`; performance health before landing | reliable DONE eligibility | Post-land process squad; dedicated Integrator | 2026-08-27 | — | Remediation head `557af45b`: `-NoImport` records structured red, forces import and remains ineligible; descriptor Windows debug target is parsed/path-bound/hash-bound to the actual loaded DLL; hostile byte-identical duplicate path fails closed. Validation PASS 49.3s; two tool files only. Second rejection escalates owner, never third round. |
