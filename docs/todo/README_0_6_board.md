...
| fix06_16 | `fix06_16_web_perf_server_orphan_cleanup_prompt.md` | IN_PROGRESS | fix06_14 exact-head qualification | fix06_13 unparked locked run | `/root/fix06_16_impl` | 2026-08-27 | — | Exact-base bounded reproduction confirmed stopping wrapper PID 2380 left direct Python PID 12964 listening on port 64488; only those exact PIDs were cleaned. Non-Web remediation publishes and revalidates nonce/PID/start-time/parent/script/port ownership, stops child then wrapper on every unwind and fails if the exact listener remains. Hostile lifecycle suite passed twice and static validation passed; no browser/Web timing/export ran. Independent exact-head review remains required. |
