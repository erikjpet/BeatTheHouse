...
| env06_7 | `env06_7_all_variations_sequence_rework_prompt.md` | IN_PROGRESS — C OWNER DECISION; ASSEMBLY + B REMEDIATION ACTIVE; E PARKED | accepted env06_6b plus ordered real catalog authority for landing | depth06_1 | Squads A-E + ordered assembly; all branches preserved | 2026-08-27 | — | A/D handed off; C frozen after second rejection. E clean park `08ff8e8a` includes identity-specific spatial graphs, full snapshot/observer/injection/traversal/lifecycle matrices, 80 native rasters/contact sheet, and native/Web hash parity; it is blocked only on assembly-owned Delta Queen/Grand Casino zones/anchors and three Grand Casino internal routes. Sole ordered assembly owner is active. Freed E squad rotated to Package B first-remediation for package-local lifecycle/platform/presentation proof and fail-closed production-seam consumption; no synthetic zones or shared-file edits. |
...
| game06_3 | `game06_3_baccarat_roulette_depth_prompt.md` | IN_PROGRESS — FIRST-REJECTION REMEDIATION WIP GREEN | accepted frozen contract `a2760d81`; accepted game06_1 product successor for integration | depth parity | Squad BACCARAT-ROULETTE; dedicated Integrator | 2026-08-27 | — | Rejected `2852387f` preserved; clean remediation WIP `641419cc` has focused executable contract PASS and project validator PASS. Roulette 10-seed audit completed 10 tables/surfaces/draws/spins with 96-frame trajectories; wrapper red only inherited missing-assets. Full table suite cannot parse on frozen base due inherited slot-surface helper dependency; Baccarat audit retains inherited ~250 missing assets and 40-hand undersampling. Row-local closure/evidence continues; no acceptance claim yet. |
...
| world06_4 | `world06_4_backroom_jobs_recruitment_prompt.md` | IN_PROGRESS — RECRUITMENT + DATA STAFFED / CREW WIP PARKING | world06_1 for sequence/adapter integration, review and landing | — | Codex/lane:world06_4; `codex/world06_4` | 2026-08-27 | — | Claimed at `537e3879` on clean main, excluding parked authority. Recruitment model/proof and locked-value data/proof remain independently staffed. The reassigned job-model owner is preserving its visible 232-line `crew_state_model` WIP as UNREVIEWED before world06_3 remediation. RunState/EventModule/adapter/delivery/Numbers/planning/receipt authority excluded; `stake_horse`/`collection` remain model outcomes awaiting sequences. |
