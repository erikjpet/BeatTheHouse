...
| perf06_1 | `perf06_1_finish_and_qualify_prompt.md` | IN_PROGRESS | Families 1 and 2 merged | playtest06_2 | `/root` | 2026-09-11 | — | Phases 1-3 complete on `codex/perf06-finish`: stale harness custody verified; all 18 non-measurement gates and Audit passed; Audit/Full now run 14 cheap fail-closed contracts. Reduced native/Web/low-end and integration rehearsal retained honest reds, including Grand Casino's cross-layout semantic placement failure routed to active `fix06_31`; two launcher provenance/argument defects were fixed permanently. Binding run estimate is 8-12 uninterrupted hours. Phase 4 remains NOT RUN: `origin/main` is still `c570f2ce`, `fix06_31` is unlanded, Chrome is active, and no distinct director witness was supplied. Report: `../plans/perf06_1_performance_platform_report.md`. |
