...
| fix06_22 | `fix06_22_fail_closed_post_land_verification_prompt.md` | IN_PROGRESS | exact current main `a62f4884`; preserved process WIP `c1d80ff0`/`41d54f61` | reliable DONE eligibility | Post-land process squad; dedicated Integrator | 2026-08-27 | — | Complete Smoke only; exact start/end main/tree, clean tracked checkout, Godot/native source/descriptor/built DLL identities, structured `eligible_for_done=false` on every red. Focused gates may never substitute. No product/budget/assertion change. |
| fix06_23 | `fix06_23_main_performance_health_evidence_prompt.md` | IN_PROGRESS — CAPACITY QUEUED | exact main `a62f4884` foundation performance red | Smoke-green main; env06_6 landing | next evidence squad; dedicated Integrator | 2026-08-27 | — | Evidence-only idle-host reproduction. Full retained red has 15 findings; targeted controls were green on `3f5e9907` and `e3f6e94a`, targeted `a62f4884` red on pusher sequence. Source and merge trees are identical. No code/test/budget/product change authorized. |
