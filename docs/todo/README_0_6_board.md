...
| craps06_3 | `craps06_3_craps_depth_rework_prompt.md` | ACCEPTED-READY — INTEGRATOR INTAKE | craps06_1/2; env06_6b for landing; owner-frozen game ritual contract | depth06_1 | Squad CRAPS06_3; dedicated Integrator | 2026-08-27 | — | Product `f5a30b19`; immutable handoff `1b8ae52c`, clean/diff-check clean, five row-local payloads only. Validation and exact row contract PASS; focused validation/import/load PASS; million-roll RTP/fairness/street parity functionally PASS; actual-GL 1280×720 surface retained/readable. Disclosed foundation red is inherited env lifecycle/routes plus two stale shared immediate-resolve/`rolling` assertions conflicting with the frozen offer/throw and bounce/read/dealer contract. No env/crew/world/shared assembly/test/master edits. |
...
| game06_3 | `game06_3_baccarat_roulette_depth_prompt.md` | IN_PROGRESS — CONTRACT-ONLY / REJECTED RUNTIME FORBIDDEN | accepted frozen contract `a2760d81`; accepted game06_1 product successor for integration | depth parity | Squad BACCARAT-ROULETTE | 2026-08-27 | — | Row-local work may continue against the accepted frozen contract only. Rejected runtime `44fefe5f` must not be imported, copied, or treated as authority; dependent WIP must park clean if encountered. |
