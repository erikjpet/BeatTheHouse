...
| craps06_3 | `craps06_3_craps_depth_rework_prompt.md` | IN_PROGRESS — PRODUCT FIRST REJECTION / SPLIT REMEDIATION | craps06_1/2; env06_6b for landing; owner-frozen game ritual contract | depth06_1 | Squad CRAPS06_3 + `codex/craps06_3-env-integration`; dedicated Integrator | 2026-08-27 | — | Exact rejected product `f5a30b19`/handoff `1b8ae52c` preserved. Original squad owns tactile travel/bounce/separation/offer-return, phase, executable proof, and registered-suite remediation. Exclusive env lane has a separate branch/worktree based on immutable handoff and owns registration, `craps_public_facts` routing, five environment-profile bindings, and unique executable proof; first UNREVIEWED WIP due 22:32 CDT. Frozen env06_6 `855a2961` and env06_6b `b7da64ae` remain untouched. Integrator implements nothing. |
...
| ID | Prompt | Status | Depends on | Unblocks | Agent | Started | Finished | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| game06_1 | `game06_1_table_machine_ritual_runtime_prompt.md` | IN_PROGRESS — FIRST-REJECTION REMEDIATION WIP GREEN | accepted frozen contract `a2760d81`; red main blocks landing only | game06_2..7 | Squad GAME06_1; dedicated Integrator | 2026-08-27 | — | Rejected `44fefe5f` preserved; remediation checkpoint `c5e5d4d5` is durable UNREVIEWED with focused suite PASS. It adds complete RitualCommand intake, live authenticated-origin equality, supplied canonical fingerprint, closed result/rejection/cache envelopes and taxonomy, per-handler operation/fact allowlists with hostile cross-handler proofs, and authenticated closed restore with hostile unknown/type/reference/fingerprint/cache/schema/sequence/version/migration proofs. Contract untouched; exact gate hardening continues before handoff. |
...
| game06_3 | `game06_3_baccarat_roulette_depth_prompt.md` | PARKED — SLOT RETURNED TO CRAPS FIRST REJECTION | accepted frozen contract `a2760d81`; accepted game06_1 product successor for integration | depth parity | Squad BACCARAT-ROULETTE; `codex/game06_3-impl` | 2026-08-27 | — | Clean UNREVIEWED `001f3c13`, based directly on frozen contract `a2760d81` and repointed away from rejected runtime before edits. One self-contained row-local `roulette.gd` projection (103+/1-), diff-check PASS; compile/tests not run due immediate park. Squad returned to craps06_3 remediation. |
