...
| fix06_13 | `fix06_13_coin_pusher_shipped_web_performance_prompt.md` | SECOND-REVIEW ACCEPTED / FUNCTIONAL QUALIFICATION ACTIVE | accepted fix06_9 evidence remediation; fix06_14/16 DONE | fix06_9, pusherv3_11 closure | Squad FIX06_13; Second Integrator | 2026-08-27 | — | Exact successor `0414cefc`/tree `2d37dcde` independently ACCEPTED: fresh empty member Dictionary releases transient context without mutating caller storage; both cached paths prove caller bytes/RNG identity then weakref death. Integrator is rebuilding/hashing and running one functional qualification. Only after functional green may the still-unconsumed single locked Web run execute; a qualifying result precedes no-ff landing and functional PostLand. Frozen reds remain provenance. |
...
| fix06_24 | `fix06_24_coin_pusher_adjacent_coin_movement_defect_prompt.md` | BLOCKED — PRODUCT-PHYSICS DEFECT / OWNER DECISION | exact fix06_8 evidence `1f0595af` | pusherv3_11 | evidence squad; owner authorization required | 2026-08-27 | — | Corrected docs-only option-neutral packet exact `a761f2c5` supersedes stale `d2a860da`: the prompt is absent from main `00ee744f` but committed on the program-board lineage at `a3865e4d`/blob `8011fa54`. Choice remains A authorize a separately bounded product-physics remediation, or B revise/cut Plan 9.4. No selection, rerun/search or physics/geometry/nozzle/transport/contact/neighboring-stock authority exists; independent docs review is queued. |
