...
| fix06_13 | `fix06_13_coin_pusher_shipped_web_performance_prompt.md` | TODO | accepted fix06_9 evidence remediation | fix06_9, pusherv3_11 closure | — | — | — | Routed product defect: first fresh Chrome CPU-4 shipped-Web measurement at exact 300-body origin recorded ~142–145 ms frame p95, ~52–55 ms draw p95 and 45–91 ms synchronous action costs against locked 16/5 idle, 22/7 active and 16 ms resolve caps. |
| fix06_14 | `fix06_14_coin_pusher_collect_reinstall_clock_evidence_prompt.md` | IN PROGRESS / RE-REVIEW PENDING | fix06_13 shipped-Web evidence | fix06_9 evidence closure | `/root/fix06_14_clock` | 2026-08-27 | — | Retained evidence proves four clock-boundary mismatches without a simulation defect: a 120-frame CPU-4 idle lasted 7.587s and produced seven valid 1 Hz scheduler redraws against a frame-derived floor of eight; reduced motion intentionally disabled presentation scheduling while live solver/state-driven draw evidence continued; reinstall identity was sampled only after live entry frames; and COLLECT action completion was overwritten by legitimate exits during the subsequent 60-frame observation. Reviews rejected `38a1459d` for comparing a later reduced sample to 300 and omitting terminal channels, then rejected `450803d4` because surface counts could disagree with arithmetic evidence and a missing solver invariant defaulted green. Remediation `b044684e` binds every surface body/tray/liveness value to its conservation snapshot and fails closed unless the solver invariant is present and true. No cap, fixture, sample length, action, simulation or economy behavior changed. Independent re-review and exact fix06_13 integration remain required. |
