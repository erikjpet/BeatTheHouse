...
| fix06_8 | `fix06_8_coin_pusher_upper_row_join_evidence_prompt.md` | IN_PROGRESS | `pusherv3_10` landed | Plan 9.4 closure evidence | PM:Codex/sub:program_row_inventory | 2026-08-26 | — | Replace the stale direct-solver 70-body `upper_row_join` capture with deterministic production-entry, production-drop, idle-control and local phase-matched neighbor evidence for all three shipped variants. Evidence tooling only; a valid trace with no local advance is routed as a product defect. |
