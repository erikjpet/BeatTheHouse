...
| fix06_17 | `fix06_17_scratch_resolve_tail_timing_prompt.md` | TODO / ROUTED | retained `fix06_13` current-main native performance result | — | `/root/fix06_17_scratch_timing_route` | 2026-08-27 | — | Nonblocking routed timing defect. Exact `cce1c8df` actual_1 had one failure: Scratch resolve max `12.046/6.000 ms`; avg/p95 were green at `2.34789583333333/3.734 ms`. Report SHA-256 `4195D724...6090EA`. Historical isolated spikes plus green five-run series classify this as measurement/tail variance, not a reproduced product regression, but the red remains preserved. No rerun, cap/test/product change or waiver occurred; `fix06_13` Coin Pusher scope was entirely green. |
