...
| fix06_13 | `fix06_13_coin_pusher_shipped_web_performance_prompt.md` | PARKED | accepted fix06_9 evidence remediation | fix06_9, pusherv3_11 closure | `/root/fix06_13_web_perf` | 2026-08-27 | — | Off-main implementation `7ec148e4d9a6096627fa26e1afee508e5b1c0b25` and exact review/docs head `718af3da7176abfaec9f9dbc10884454298a9872` were independently accepted, but nothing from that branch is accepted on or transplanted to `main`. The sole accepted-head locked run at `c914546f` remained red and exposed clock/fixture evidence contradictions now owned by `fix06_14`. Unpark only after `fix06_14` lands, then integrate semantically onto current `main`, run current-main gates, obtain fresh independent review, and consume one new locked shipped-Web run. Every red and hash remains retained on `codex/land06-fix06_13` and under its ignored evidence paths. |
| fix06_14 | `fix06_14_coin_pusher_collect_reinstall_clock_evidence_prompt.md` | IN PROGRESS / INTEGRATION REVIEW PENDING | fix06_13 shipped-Web evidence | fix06_9 evidence closure | `/root/fix06_14_clock` | 2026-08-27 | — | Retained evidence proves four clock-boundary mismatches without a simulation defect: a 120-frame CPU-4 idle lasted 7.587s and produced seven valid 1 Hz scheduler redraws against a frame-derived floor of eight; reduced motion intentionally disabled presentation scheduling while live solver/state-driven draw evidence continued; reinstall identity was sampled only after live entry frames; and COLLECT action completion was overwritten by legitimate exits during the subsequent 60-frame observation. Five reviews successively closed later-sample identity, conservation, terminal binding, missing-field casts and enclosing reduced-object guards. Remediation `9256e314` requires every reduced after/scheduler/draw object and field before any cast, in addition to exact nonnegative channels and seeded COLLECT terminal accounting. Final independent re-review accepted exact source head `d0306126bbe8110bb6d4ef79b335c6dcdd3dcc31`; current-main integration review and the later fix06_13 integration remain required. No cap, fixture, sample length, action, simulation or economy behavior changed. |
