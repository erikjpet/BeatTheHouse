...
- Rooms — Lane A 2026-09-24 21:14 CT. Works now on `main`: physical-only fixed slots and action-list overflow, true counter occlusion, reachable travel fallback, rebuilt Bar/Corner Store/Grand Casino layouts, distinct Grand private-room art, grounded hand-authored slots across every remaining public, private, and home map, separated Beach, Delta Queen, Jazz, and Gas Station contacts, a distinct Kitty Cat signup surface and grounded judge pinned across scene states, a dedicated Motel counter-phone surface clear of the exit, all three reachable Punchline rooms with scenarios installed on their authored layers, Punchline abstract tasks kept exclusively in the room-action list, fitted long room titles, and an adaptive no-scroll action grid. The final normal-use pass exited `0`; `q008_rooms.png`, `rooms_A.png`, and the 20-map `rooms_all.png` were regenerated and opened, while `room_actions_dense.png` shows all 11 dense-room actions simultaneously visible with readable labels and no scroll controls. Q-021 remains OPEN at `D:\Projects\Beat-The-House\.tmp\owner_review\q008_rooms.png`. Next: obtain owner approval and apply only any blocking layout feedback.
- Endings — Lane B 2026-09-24 21:13 CT. Works now on `main`: the isolated normal-play route handles Linda's immediate service and off-canvas Cashout Counter through the visible room-action list, blank-room refocus, crowded-room overflow, capped-map scouting, and visible multi-leg return routes. Successful Crew packages clear exactly one favor at their transactional boundary, River Queen's dormant scenario rectangles reconcile before arrival, Grand Casino subroom returns align their scenario layout baseline before install, and Linda's fully rendered TalkDock reserves disabled multi-line choice height. Clean has traversed the formerly clipped Players Card Back control twice, reached Grand Casino, exchanged chips, and completed a blackjack hand without a route assertion. Ordinary blackjack decisions use the rendered active total, and explicit waits temporarily suspend then exactly restore every observed application-lifecycle pause owner, so unattended focus loss no longer freezes deal or payout presentation. The confirmation wrapper now accepts a 30-minute deadline instead of rejecting anything above 300 seconds; the narrow timeout fix is merged and pushed as `920cdf14`. Heist now resolves the exact authored post-Audit Collector TalkDock with deterministic `promise`, preserves route cash, and leaves any chained Crew favor for the existing `run_package` flow. Next: finish Lane B's normal Clean and Cheat confirmations with 1200-second deadlines and save/open `ending_clean.png` and `ending_cheat.png`; Crew Heist and `ending_heist.png` belong to Lane C part 3.
