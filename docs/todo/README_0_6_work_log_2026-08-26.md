...
* 2026-09-03 — `integ06_1` COMPOSITION/SOAK CLAIM: `/root/world_closeout` began the opt-in maximal-node and full-run soak lane from exact integrated head `15c1c688`; `/root/integ06_1_fixtures` separately owns genuine v0.5.1 fixtures and migration. This lane will recover and extend the landed Foundation soak, scenario, depth, and world probes for all viable archetypes/Punchline layers, lifecycle orderings, mid-composition save/load, terminal coverage, crew-ignoring control, exact native/Web traces, and bounded state/frame/memory. The overall row remains IN_PROGRESS until both lanes combine and final gates pass.
