...
* 2026-09-06 — `env06_8` STALE-BRANCH DECISION DEFAULT: recorded the open question about the materially different contract and extra geometry/hidden-boundary/contact-sheet tools retained on `codex/closeout06-final`. Applied the controlling prompt's default of no port and preserved that branch unchanged.
...
* 2026-09-14 — OWNER PLACEMENT DECISION: the reusable spawn-slot/fixture placement experiment is rejected for integration. Its source, tests, evidence index and decision notes are preserved on `codex/reusable-environment-slots` at `b182b3c6`; none of it is merged to `main`. Room construction must be reconsidered for the expanded object inventory before `perf06_1`, `playtest06_2`, polish, or release work resumes.
