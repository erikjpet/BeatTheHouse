...
| --- | --- | --- | --- | --- |
| 0c | playtest_beach_placement_duplicate_objects_prompt.md | claimed (2026-07-08, Codex PM machine) | PM machine | Playtest blockers: beach must always anchor 1 distance block from delta_queen with a direct edge (it currently falls through the anchor table to the hash fallback), and identity-carrying objects (clerks etc.) must be unique per environment via a data-driven uniqueness class (owner hit two pull-tab clerks). Adds seed-sweep guards to the generation audit. |
