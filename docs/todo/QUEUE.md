...
| --- | --- | --- | --- | --- |
| 0f | playtest_bankroll_spoiler_prompt.md | ready | PM machine | Playtest blocker: bankroll updates at bet settle (action time) instead of at result presentation, spoiling outcomes. Presentation-layer fix only — a presented-bankroll display value syncing at reveal boundaries; simulation/settle timing untouched (determinism). Stake deduction stays immediate; guard samples the bar mid-animation. |
