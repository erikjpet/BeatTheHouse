...
- [x] Counter-game consumers explicitly select their release availability.
  All seven aligned Scratch families are active. Crossword's v4→v5 machine
  migration converts the former held availability marker to active while
  preserving saved stock quantity and every issued-ticket lifecycle byte.
