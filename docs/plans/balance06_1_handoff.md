...
- `docs/plans/evidence/balance06_1/measurements/` — smoke, determinism first, and
  byte-identical determinism repeat JSON.
...
- `docs/plans/evidence/balance06_1/SHA256SUMS.txt` — hashes the complete 65-file
  source artifact set.
- `.tmp/balance06_1/` — the original ignored files are intentionally preserved
  in place; the committed archive is byte-for-byte identical and makes them
  collectable from the branch.
