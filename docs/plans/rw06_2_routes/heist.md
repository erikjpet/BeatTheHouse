...
route: a public Crew marker used only to fund the first Grand trip, the earliest
public Grand hook read, both favor deliveries, Punchline discovery, Bishop's
two-beat recruitment and visible job ladder, three distinct Grand identity
visits, real Cage/main-room setup travel,
the three live-table decisions, and a real getaway delivery. Before launching
Godot, its production-tree preflight replays the visible START defaults,
...
games, and a cage interview. Plan B remains outside this qualifying replay and
has no proved fallback route. The runner fails closed if Plan A is absent; it
never silently chooses Plan B. Plan A is the narrower candidate for a 150–350
action release replay.

## Source-derived route

1. Start a normal seeded run only from the exact visible START defaults: Standard
   challenge, Random home, and the ordered default content groups. Include the
   real fresh-profile meta bag/home modifiers in the challenge key. Never model
   this as an empty modifier string.
2. At the guaranteed Corner Store, accept and confirm the visible two-favor Crew
   marker. Verify the exact $45 bankroll increase and one rendered debt-count
   increase; this funds the first Grand trip without accepting another lender.
3. Reach Grand Casino Main before the Bishop grind, require the exact rendered
   **The Audit Roster** event, and resolve **Read the shift**. Fail immediately if
   the hook is absent, clipped, disabled, duplicated, or only present in seeded,
   stored, or other private scenario state.
4. Clear both Crew favors. For each favor, pay the rendered and enabled **Cashier
   Tip** only until the public favor surfaces (at most two), verifying every exact
   $4 bankroll decrease, then complete its delivery. The invitation/Audit choices
   may already have consumed an action boundary. Travel advances the clock only
   and is forbidden as a marker-aging fallback.
5. Discover the Punchline through Parking Lot Tip and Side Door, then meet/recruit
   Bishop through his visible two-beat appointment (**Wait by window**, then
   **Keep appointment**). Recruitment promotes Bishop to Associate (30 trust).
   Do not abandon jobs, make false accusations, or create other grievances.
6. Raise Bishop from Associate to Inner Circle (90) through visible Bishop jobs
...
7. Treat `RW06-HEIST-AUDIT-0013` as a rejected stale pin: the real fresh-profile
...
15. Keep heat controlled, then choose **Take the exit**. Both cart exits converge
...
16. Confirm an ended run and a visible Crew heist outcome (`clean_sweep`,
...
- If The Count cannot be made naturally reachable within the target run length,
  stop and classify the Plan A product/route blocker. Do not inject Audit Night
  or Bishop trust, do not fall back to unproved Plan B, and do not silently
  redefine completion.
